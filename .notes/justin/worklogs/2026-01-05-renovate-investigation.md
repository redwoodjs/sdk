...
- 2026-01-05: Investigated why Dependabot alerts do not always result in Renovate vulnerability PRs. Past Renovate PRs like `renovate/npm-astro-vulnerability` and `renovate/npm-vite-vulnerability` indicate Renovate can create vulnerability PRs. The missing case (Dependabot alert #128 for `qs` from `pnpm-lock.yaml`) appears to be a transitive-only vulnerability where the vulnerable package is not a direct dependency in any manifest, which limits how Renovate can propose a targeted fix without an explicit parent bump or an override mechanism.
- 2026-01-05: Decided on a split: Dependabot for security remediation PRs (including transitive lockfile fixes like `qs`), Renovate for grouped/scheduled dependency updates. Updated `renovate.json` to disable Renovate vulnerability PRs (`vulnerabilityAlerts.enabled = false`) to reduce duplicate security PRs.
