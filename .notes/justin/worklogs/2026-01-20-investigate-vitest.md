...
## Decision: Clean Up & Document Showcase

We have decided to proceed with the **Test-Bridge Showcase** as the canonical example for testing in RedwoodSDK (for now). We will not pivot to the Pure Plugin approach.

### Tasks
1.  **Refine the Showcase (`vitest-showcase`)**:
    *   Clean up the existing `sdk_hs-vitest-playground/playground/community/vitest-showcase`.
    *   Remove `vitest-plugin-rsc` (redundant for built worker approach).
    *   Remove unnecessary test complexity (MSW, bindings-check, unused console logs).
    *   Revise README to clearly state the purpose (Showcase for Integration Testing).
2.  **Document the Strategy**:
    *   Add a new document in `docs/src` explaining the "Built Worker Integration Test" pattern.
    *   Document the limitations (Requires build, no internal mocking).
    *   Provide a "Recipe" for setting up the bridge (`handleTestRequest`, `invoke`).

### Context
We are working across two worktrees:
*   Logic/Docs: `sdk_hs-vitest-playground`
*   Worklog: `sdk_investigate-vitest`
We will sync the worklog to the playground worktree at the end if needed.

## Implementation Update

### Showcase Cleanup
Cleaned up `playground/community/vitest-showcase`:
*   Removed `vitest-plugin-rsc` and browser-mode dependencies.
*   Standardized on `vitest-pool-workers` configuration (`vitest.config.ts`).
*   Removed unused/confusing tests (`msw`, `bindings`, `app.worker`, `server.test`).
*   Consolidated test logic into `rsc.worker.test.ts` demonstrating the Bridge pattern.
*   Implemented `initDatabase` in `test-utils.tsx` to support the component test.
*   Updated `README.md` to reflect the "Test-Bridge" purpose.

### Documentation
Added `docs/src/content/docs/guides/testing.mdx`:
