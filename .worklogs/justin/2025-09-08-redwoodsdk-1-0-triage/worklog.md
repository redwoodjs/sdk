...
All triage decisions will be recorded in [`./output-issues.md`](./output-issues.md) to prepare for an automated script that will update GitHub. The triage notes will be written in a direct, public-safe tone suitable for posting as a comment on the relevant GitHub issue.

*   **For existing GitHub Issues**: Record the issue number, the assigned label(s), and a 1-2 sentence triage note. The script will use this to apply the label and post the triage note as a comment.
*   **For new issues** (from to-do list items or missing tasks): Propose a new issue with a title, description, the assigned label(s), and a triage note added as a comment. The script will use this to create a new issue.
