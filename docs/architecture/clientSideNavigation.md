...
6.  **Scroll Management**: The runtime takes manual control of scroll restoration by setting `history.scrollRestoration = "manual"` during initialization. The browser's default `"auto"` behavior restores scroll position immediately on `popstate` — before the RSC payload has fetched and committed — which causes the old DOM to flash at the new scroll offset. Under manual control, a scroll *intent* is recorded at navigation time (scroll-to-top for link clicks and `navigate()` calls; the saved scroll position read from `history.state` for `popstate`) and applied post-commit via the `onHydrated` hook, so the scroll adjustment lands together with the new DOM. To make back/forward restoration accurate, a throttled (rAF-coalesced) `scroll` listener persists the user's current scroll position onto the current history entry via `replaceState`, and `navigate()` also saves the outgoing scroll position before `pushState`. On initial boot, if the entry already has saved scroll state (for example, after a reload on a scrolled page), that position is queued and applied on first hydration. Scroll-to-top behavior can be configured to use smooth scrolling or can be disabled entirely.
