...
6.  **Scroll Management**: The runtime takes manual control of scroll restoration by setting `history.scrollRestoration = "manual"` during initialization. The browser's default `"auto"` behavior restores scroll position immediately on `popstate` — before the RSC payload has fetched and committed — which causes the old DOM to flash at the new scroll offset. Under manual control, a scroll _intent_ is recorded at navigation time (scroll-to-top for link clicks and `navigate()` calls; the saved in-memory scroll position for `popstate`) and applied post-commit via the `onHydrated` hook, so the scroll adjustment lands together with the new DOM. To make back/forward restoration accurate without triggering browser throttling, each history entry gets a lightweight key and hot scroll updates are tracked in memory instead of being written to `history.state` on every scroll. The latest position is still flushed to `history.state` at navigation boundaries and when the page is hidden/unloaded, so reloading a scrolled page can restore the saved position in the next session. Scroll-to-top behavior can be configured to use smooth scrolling or can be disabled entirely.
