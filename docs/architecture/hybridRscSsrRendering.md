...
This holding behavior makes decoding stateful, so the code follows a one-to-one pairing rule: each stream gets exactly one decoder, used for that stream's entire life. Breaking the pairing in either direction corrupts the text. If the code decodes chunks while searching for a marker and then forwards the remaining chunks as raw bytes — the tempting shortcut, since those chunks no longer need to be searched — any bytes the decoder is holding at that moment are silently lost, and the browser renders the orphaned remainder as replacement characters (`�`). And if one decoder serves two streams, it attaches a held fragment from one render to the text of the other, corrupting both.

The pre-processing step applies this pairing to the application stream — one decoder from start to finish: the chunks after the hoisted-tag block are decoded and re-encoded unchanged rather than forwarded as raw bytes, and the decoder is flushed, emitting any bytes it is still holding, when the stream ends. Since every emitted chunk is re-encoded from complete text, each chunk of the two resulting streams ends on a character boundary; Phase 5 below relies on this guarantee to pass application chunks through without decoding them again. The stitcher applies the same pairing to its three input streams, flushing each decoder when its stream ends:
