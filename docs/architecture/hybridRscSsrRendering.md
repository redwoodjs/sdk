...
Because the translator can be holding a piece of a character at any moment, two rules keep the text intact. First, every chunk of a stream must go through the same translator, from the first chunk to the last: if earlier chunks are translated so the utility can search them for markers, but later chunks are shipped onward untouched, the piece the translator was holding is lost, and the browser receives the tail of a character it never saw the start of and shows `�` in its place. Second, each stream needs its own translator: if the document stream and the application stream shared one, a piece of a character from one stream could be completed with bytes from the other, corrupting both.
