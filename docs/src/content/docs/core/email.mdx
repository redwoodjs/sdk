...
RedwoodSDK integrates with [Cloudflare Email Routing](https://developers.cloudflare.com/email-routing/email-workers/) to send, receive, reply, and forward email directly from your Worker.
This guide walks through the required configuration for production deployments, how to exercise the email handler locally,
and provides a reference worker illustrating the common patterns.
