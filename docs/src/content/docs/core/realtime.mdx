...
export const useSyncState = createSyncStateHook("/sync-state", {
  useState,
  useEffect,
  useRef,
  useCallback,
...
`createSyncStateHook(url?, deps?)` accepts two optional arguments:

- `url`: overrides the HTTP route used by the client. The default is `/__sync-state`.
- `deps`: overrides the React primitives. Pass this when integrating with a custom renderer or testing environment.

```ts title="src/hooks/useCustomSyncState.ts"
const useCustomSyncState = createSyncStateHook("/api/sync");
```

```ts title="src/hooks/useSyncState.test.ts"
const useTestSyncState = createSyncStateHook(undefined, {
  useState,
  useEffect,
  useRef,
  useCallback,
