...
`useSyncedState` keeps a value aligned across tabs, devices, or users by using the realtime transport and the `SyncStateCoordinator` Durable Object.
...
`SyncStateCoordinator.registerSetStateHandler(handler)` accepts a function that receives `(key, value)` each time the coordinator stores a new value. Use this to record updates or trigger downstream work whenever any client publishes new state.

`SyncStateCoordinator.registerGetStateHandler(handler)` accepts a function that receives `(key, value)` each time the coordinator returns a value to a subscriber. The handler runs after the lookup, so `value` is `undefined` when no entry exists for the key.

### Durable Object Export

```tsx title="src/worker.tsx"
export { SyncStateCoordinator } from "rwsdk/use-sync-state";
```
...
```tsx title="src/worker.tsx"
import { SyncStateCoordinator } from "rwsdk/use-sync-state/worker";
import { sessionStore } from "./session";
import { requestInfo } from "rwsdk/worker";

SyncStateCoordinator.registerKeyHandler(async (key) => {
