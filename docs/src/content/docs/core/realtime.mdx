...
`useSyncState` keeps a value aligned across tabs, devices, or users by using the realtime transport and the `SyncStateCoordinator` Durable Object.

### Default Hook

```ts title="src/hooks/useSyncState.ts"
import { useSyncState } from "rwsdk/use-sync-state";
```
...
`registerSetStateCallback(onSetStateHandler)` accepts a function that receives `(key, value)` each time the coordinator stores a new value. Use this to record updates or trigger downstream work whenever any client publishes new state.

`registerGetStateCallback(onGetStateHandler)` accepts a function that receives `(key, value)` each time the coordinator returns a value to a subscriber. The handler runs after the lookup, so `value` is `undefined` when no entry exists for the key.
...
```ts title="src/hooks/useSyncState.test.ts"
