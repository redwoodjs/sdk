...
`useSyncedState` keeps a value aligned across tabs, devices, or users by using the realtime transport and the `SyncedStateCoordinator` Durable Object. `useServerState` is an alias kept for compatibility.

### Hook Factory

```ts title="src/hooks/useSyncedState.ts"
import { useState } from "react";
import { createSyncedStateHook } from "rwsdk/use-server-state";

export const useSyncedState = createSyncedStateHook(useState);
```

### Worker Routes

```tsx title="src/worker.tsx"
import { syncedStateRoutes } from "rwsdk/use-server-state/worker";
import { env } from "cloudflare:workers";
...
```tsx title="src/worker.tsx"
export { SyncedStateCoordinator } from "rwsdk/use-sync-state";
```
