...
The request/response cycle is at the heart of web development - when a browser makes a request, your server needs to respond with content. RedwoodSDK makes this easy with the `defineApp` function, which lets you elegantly handle incoming requests and return the right responses.
...
Context is a mutable object that is passed to each request handler, interruptors, and React Server Actions. It's used to share data between the different parts of your application. You populate the context on a per-request basis via Middleware.
...
export default defineApp([layout(Document, [route("/", AboutPage)])]);
```
...
<Aside type="note" title="Client Side Hydration">
  ...
  the React components will not be hydrated on the client. This also means that
  you have a massive amount of control over the HTML returned by Redwood.
</Aside>
