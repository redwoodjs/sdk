...
The request/response cycle is at the heart of web development - when a browser makes a request, your server needs to respond with content. @redwoodjs/sdk makes this easy with the `defineApp` function, which lets you elegantly handle incoming requests and return the right responses.
...
Layouts are a way to wrap your pages in a consistent layout. In @redwoodjs/sdk, layouts are defined using the `layout` function in `defineApp`.
...
<Aside type="tip" title="Layouts">
  @redwoodjs/sdk does not supply a default "html" template. You must supply your
