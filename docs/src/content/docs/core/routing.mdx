...
`linkFor` exposes the full set of routes discovered inside `defineApp`. TypeScript verifies that the path you pass exists and that you provide the required parameters.

### Type Safety

If you reference a path that your app does not define, TypeScript raises an error at compile time.
...
The helper enforces the parameter shape implied by each route pattern.

For named parameters, pass an object whose keys match the parameter names:
...
For wildcard parameters, use `$0`, `$1`, and so on, following the order of the wildcards from left to right:
...
- **path**: `string` — A route pattern defined in your app.
- **params** (optional): `Record<string, string>` — An object for dynamic segments.
  ...
  - For wildcard parameters (e.g., `/files/*`), keys should be `$0`, `$1`, etc., in the order the wildcards appear (e.g., `{ $0: "image.jpg" }`).

#### `linkFor<App>(): function`
Returns a `link` helper whose path union is inferred from the `defineApp` routes. Use `import type` to reference the app without bundling the worker.

#### `createLinks<App>(): function`
Alias of `linkFor`; kept for symmetry with the legacy API.

#### `defineLinks(routes): function`
Manual escape hatch that accepts a route list when you cannot reference the app type directly (for example, in isolated tests).
