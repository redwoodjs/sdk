...
Documents are how you define the "shell" of your application's html: the `<html>`, `<head>`, `<meta>` tags, scripts, stylesheets, `<body>`, and where in the `<body>` your page is rendered. In RedwoodSDK, documents are defined using the `document` function in `defineApp`.
