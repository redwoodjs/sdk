...
### Preloading navigations with `<link rel="preload">`
...
export function AboutPageLayout() {
  ...
  return (
    <>
      ...
      <link rel="preload" href={aboutHref} />
      {/* ...rest of your page... */}
    </>
  );
}
...
After each client navigation, Redwood scans `link[rel="preload"][href]` elements
and issues background `GET` requests for those route-like URLs with the `__rsc`
query parameter set. Successful responses are stored in a versioned `Cache`
using `cache.put`, following the semantics described in the MDN Cache
documentation. This cache is **write-only** for now; normal navigation still
uses the standard fetch path.
