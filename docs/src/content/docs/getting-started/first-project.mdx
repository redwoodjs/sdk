...
If you're new to JavaScript frameworks or prefer a more detailed learning path, we recommend starting with our [Full-Stack Tutorial](/tutorial/full-stack-app/setup) instead.
...
The [minimal starter](/reference/starters/minimal/) ships with the least amount of code possible. This means, you don't need to delete any files or perform any cleanup to get started.
...
Here we have a `layout` function that takes the `Document` component (imported at the top of the file) and wraps our entire application. This includes our basic HTML `head` and `body` tags.
...
By default, all the pages and components in your project are server components by default. If you want to add reactivity, you'll need to use the `"use client"` directive at the top of the file.
...
Before deploying, check your `wrangler.toml` file. At the top of the file is a `name` property. If you haven't changed it, it will be `__change_me__`.
...
If you need to manage your deployed pages, you can login to your Cloudflare dashboard, expand the *Computer Workers* section, and select click on **Workers and Pages.**
