...
<Aside type="tip">
  You might have noticed that we returned JSX instead of a Response object. This is because Redwood has built-in support for React Server Components, allowing you to return JSX directly from your routes. The JSX will be rendered on the server and sent to the client as HTML.
</Aside>
