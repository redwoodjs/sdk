...
Let's talk through the logic that we're implementing:

1. When the user registers, they'll provide an email address.
2. We'll add the user to the database, and generate a verification token and expiration date.
3. The user will receive an email, asking them to verify their email address. This is important. Since this authentication system is entirely based on emailing tokens, we need to ensure that the email address they provide is valid.
4. The user clicks the link in the email that includes the token.
5. If this the first time the user is logged in, we'll check the token against the database, and if it's valid, we'll mark the user as verified and create a session for them inside a Durable Object.
6. If the user has logged in before, there's no need to verify them.  But, we'll still check the token against the database, and if it's valid, we'll create a session for them inside a Durable Object.
7. When the user wants to log out, we'll remove the session from the Durable Object.

<Aside type="note" title="Taking this one step further">
You could take this flow a step further and set up a cron job to remove unverified users after a certain period of time.
</Aside>
...
+  @@index([username, email, magicLink])
}
...
You'll notice we also created an `@@index` for the username, email, and magicLink fields. This will help us speed up our queries, since we'll need to look up our users by username and/or email.
...
```bash
pnpm migrate:new "update the user table with email and magic link fields"
```
