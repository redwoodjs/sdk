...
```tsx title="src/app/pages/auth/actions.ts" {"1. Take the form data and get the email address": 2} {"2. Validate the form": 5} {"3. Look for the email address in the database": 14} {"4. If the email address is not found, return an error": 21} {"5. Create a reset token": 30} {"6. Update the user with the reset token and expire date": 33} {"7. If there was an error sending the email, return it": 51} {"8. Send the reset token to the user's email address": 42} {"8. Return a success message": 57} startLineNumber={135}
