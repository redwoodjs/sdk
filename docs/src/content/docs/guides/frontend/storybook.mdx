...
7. It also added `storybook` and `storybook-build` scripts to our `package.json` file. We can always run `pnpm storybook` to start the Storybook server, and `pnpm storybook-build` to build our Storybook site for production:
    ```json
    {
      "scripts": {
