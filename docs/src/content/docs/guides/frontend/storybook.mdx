...
<Aside type="note" title="What is Storybook?">
  <p>
    Storybook is a tool for developing UI components in isolation. It allows we to create and test components without needing to run our full application. It can also be a great way to document our components.
  </p>
  <p>
...
4. Storybook will finish installing, and start our Storybook server.
    ![](./images/sb-started.png)

5. It should automatically open our browser to Storybook, but if it doesn't, we can go to `localhost:6006` to see it.
    ![](./images/sb-served-site.png)

7. we'll also see that it added `storybook` and `storybook-build` scripts to our `package.json` file. we can always run `pnpm storybook` to start the Storybook server, and `pnpm storybook-build` to build our Storybook site for production.
    ```json
    {
      "scripts": {
...
<Aside type="note" title="Wait, Storybook is showing me some random components!">
  ...
  <p>
    Still, if we're new to Storybook, I recommend keeping them around for a bit and taking a look at the files it added. They show we how to set up a Storybook component and how to use and customize the Storybook UI.
  </p>
</Aside>
...
export function Home({ ctx }: RequestInfo) {
  return (
    <div>
      <p>
        {ctx.user?.username
          ? `we are logged in as user ${ctx.user.username}`
...
2. Save it, and go back to our Storybook site. we should see a new "Home" section in the sidebar:
...
<Steps>
1. If we're also using the standard starter, then the only thing in our database schema is a list of users. What's the most obvious thing to do? List out all the users! Add this to our `Home` component:
...
3. We need to mock our Prisma client. There are a few ways to do this, and [Storybook](https://storybook.js.org/docs/writing-stories/mocking-data-and-modules/mocking-modules) and [Prisma](https://www.prisma.io/blog/testing-series-1-8eRB5p0Y8o) both have great documentation on this.
    For the sake of this guide, we're going to do this the most straightforward way, so know that this is just the beginning. First, we need to create a mocked version of our Prisma client. Create a new file right next to our existing `db.ts` — `src/db.mock.ts`:
...
4. Now, we need to tell Storybook to use this mocked version of the Prisma client. We'll do this using [a Vite alias](https://storybook.js.org/docs/writing-stories/mocking-data-and-modules/mocking-modules#builder-aliases).<br />
    (we can instead use [subpath imports](https://storybook.js.org/docs/writing-stories/mocking-data-and-modules/mocking-modules#subpath-imports), but it requires a bit more setup — we'll need to change any existing imports.)<br />
...
6. Now, restart the Storybook server (`CTRL + C` and then `pnpm storybook`), and go back to the Storybook site. We should see our mocked list of users:
...
We're also well on our way to having a robust and well-documented and well-tested UI component library for our Redwood SDK project.
