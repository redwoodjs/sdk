...
Install the React Compiler Babel plugin, runtime, and Vite's React plugin.
...
- "Cannot find module 'react/compiler-runtime'": install the runtime with `pnpm add react-compiler-runtime` (or `npm i react-compiler-runtime`). This resolves the import injected by the compiler plugin. See the linked issue comment above.
