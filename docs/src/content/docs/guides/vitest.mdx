...
## 3. Implement the Bridge Handler

Create the `test-bridge.ts` handler. 

<Aside type="caution">
**Security Warning**: The handler explicitly checks `import.meta.env.PROD` to prevent exposing this route in production. You must ensure your test build runs in **development mode** (e.g. `NODE_ENV=development npm run build`) for this to work.
</Aside>
