...
@redwoodjs/sdk's routing system is built around the request/response cycle.
...
@redwoodjs/sdk supports three URL matching patterns:
...
When returning a JSX component, @redwoodjs/sdk will:
...
@redwoodjs/sdk's interruptors allow you to chain multiple request functions for a single route, where each function can either:
...
Route functions can return either a `Response` object or a JSX component. When returning JSX, @redwoodjs/sdk:
...
By default, @redwoodjs/sdk only renders your component's HTML without `<html>`, `<head>`, or `<body>` tags. To add these, wrap your routes in a layout:
