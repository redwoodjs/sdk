...
Take the `database_name` and `database_id` and update the values within the `wrangler.jsonc` file.
...
The `CLOUDFLARE_DATABASE_ID` should be the same as the `database_id` in the `wrangler.jsonc` file. But, you can also access this from your Cloudflare account:
