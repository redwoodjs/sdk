...
<details>
<summary>
If you want to see the full project structure, here it is:
</summary>
...
- pnpm-lock.yaml # lock file for pnpm (assuming you're using pnpm)
...
**TailwindCSS** is a utility-first CSS framework that makes it easy to style your components. This means instead of writing custom CSS files, TailwindCSS lets us style components directly in our HTML using pre-built classes like `text-red-500` or `bg-blue-100`. This makes styling faster and more consistent.
...
2. Configure the Vite Plugin
    ```ts ins="import tailwindcss from '@tailwindcss/vite'" ins="tailwindcss()," title="vite.config.mts" ins={7-9} {2, 11}
...
```tsx title="src/app/headers.ts" collapse={1-26} {29}
...
```tsx title="src/app/pages/user/Login.tsx" collapse={4-17} startLineNumber={61} {3,18}
...
  return (
...
1. Install shadcn/ui
    ...
    ```json title="components.json" startLineNumber={12} ins="app/"
    ...
    "aliases": {
...
2. Now, you should be able to add shadcn/ui components:
    <PackageManagers
      type="dlx"
      pkg="shadcn@latest"
      args="add"
...
Instead, we'll install it manually. It's built on top of the `<Popover />` and `<Calendar />` components -- and we just installed those.
...
```tsx title="src/app/pages/user/Login.tsx" showLineNumbers=false collapse={5-11} ins="Button" ins={1}
import { Button } from "@/app/components/ui/button";
...
return (
  <>
...
<Aside type="note" title="What You've Accomplished">
You've successfully created your first RedwoodSDK application 🎉  and have it running locally on your development server. Your project also includes TailwindCSS for styling and shadcn/ui for UI components. You've also customized your design system with fonts and colors to match the Applywise brand.
