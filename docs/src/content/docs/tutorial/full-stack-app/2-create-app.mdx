...
description: Create your RedwoodSDK project and set up the essential development tools. Install TailwindCSS, configure custom fonts, and add shadcn/ui components to build professional interfaces.
---
...
In this lesson, we'll install and configure:
- Set up our project and local development environment
- **[TailwindCSS](https://tailwindcss.com/)**: For styling our components
- **Custom fonts**: Poppins and Inter from [Google Fonts](https://fonts.google.com/)
...
You'll spend a majority of your time in the `src/app` folder and in the `worker.tsx` file.
...
2. Configure the Vite Plugin
    ...
    We need to add an empty `ssr` environment to prevent build errors. This is a temporary requirement that will be resolved in future versions.
    </Aside>
...
We need to update security headers to allow Google Fonts to load properly in production.
...
2. Add path aliases to `tsconfig.json`:
    ...
    ```
3. Add resolve alias config to `vite.config.mts`:
