...
- `0002_initial_migration.sql` was generated when we ran `pnpm migrate:new "initial migration"`.
</figcaption>
</figure>
...
<Aside title="Error: listen EADDRINUSE: address already in use :::9229">
If you get an `Error: listen EADDRINUSE: address already in use :::9229` error when running `pnpm seed`, it means that you're already running a worker (probably `pnpm dev`) when running the seed script.

![](./images/address-already-in-use.png)

Remember, the seed script needs a Cloudflare Worker, too. Quit `pnpm dev` and then you should be able to successfully run the seeder.
</Aside>
