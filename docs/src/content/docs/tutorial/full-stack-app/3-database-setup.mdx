...
Application Statuses are a little different, but the same principle applies. To avoid duplication, we will give statuses their own model. But, we will have a set of "static" statuses (New, Applied, Interviewing, Offer, and Rejected). We won't be adding, updating, or deleting this list.
...
| --- | --- | --- |
| `id` | `String` | A UUID |
| `salaryMin` | `String` | _Optional._ The minimum salary for the job. It might be helpful to make this a number, so we can sort the applications by salary. However, this might be a pain to format when displaying the data. |
| `salaryMax` | `String` | _Optional._ The maximum salary for the job. Same as `salaryMin`. |
...
Now, we need to translate all this information into a Prisma model. Prisma has it's own syntax for defining models. But, you'll notice that it looks similar to an object definition:
...
{/* TODO: Update the database diagram to include the Credential table for pass keys */}
...
Let's create the migration by running `pnpm migrate:new` and giving the migration a name. Since this is our first migration, we'll call it our `setup all database models`.

```shell
pnpm migrate:new "initial migration"
```

![]() - _TODO: Screenshot of the Terminal_

Our generated `sql` file will be in the `migrations` folder.

![]() - _TODO: Screenshot of Cursor, include the `migrations` folder and the code that was generated_

<Aside type="tip" title="The Number of Migrations">
  Worried about the number of migrations in your project? Don't. It really doesn't have an effect on your project.
</Aside>

Now, we need to apply or run the migration.
...
<Aside type="note" title="Working with other people">
  ...
  Keeping another team member's database in sync is as simple as running the migration.
</Aside>

## Previewing the Migration

If you want to preview the database, there area a couple of different options.
...
The viewer will only let you preview the database. If you want to create, update, or delete, you'll need a pro account.

### Bee Keeper Studio

[Bee Keeper Studio](https://www.beekeeperstudio.io/) is another option. It's a standalone application, but the free version will allow you to create, read, update, and delete data.

![](./images/beekeeper-setup.png)

When setting up the connection, select **SQLite** from the **Connection Type** dropdown. Then, click on "Choose File" and navigate to the .sqlite file.

![](./images/beekeeper-preview.png)

{/* TODO: Change the database screenshot so that it is exactly what the user is seeing (no data) */}

We don't have any data within our database, but you should be able to see all the tables the columns inside.

## Seed the database

Instead of manually adding data to the database, we can create a **seed file** to programmatically add data for us. This is particularly helpful if you need to reset the database during development and don't want to manually enter your test data again and again.

Within the `src` directory, create a new folder called `scripts`. inside, create a file called `seed.ts`.
...
You can copy and paste the following code inside:
...
Before we add anything to this file, let's make sure we understand what's happening:
...
* **On line 7-14**, we're using raw SQL to delete all the existing data from the database. This will allow us to start fresh when seeding the database.
* **On line 16-24**, we're adding all the job application statuses to the database. You'll notice this is a standard Prisma `createMany` function.
* **On line 26**, we're logging a message to the console saying the seeding is complete.
...
{/* TODO: Add a screenshot of this error */}
...
![](./images/seed-database-in-sqlite.png)
