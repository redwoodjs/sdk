...
First, let's set up our database. You'll need a [Cloudflare](https://www.cloudflare.com/) account. We're using their d1 services to handle the database.
...
Copy the database ID and it into your `wrangler.toml` file:
...
If you ever need to find this value, you can log into your Cloudflare account and navigate to the D1 section.

![]() - _TODO: Screenshot of the Cloudflare Dashboard_
...
This will set up Prisma and even though we're not ready to migrate our database, `migrate:dev` will create a `.wrangler` file within our project and setup the Cloudflare d1 database locally.

![]() - _TODO: Screenshot of the Terminal_
![]() - _TODO: Screenshot of the .wrangler folder_
...
;tk - this means that we didn't have to write any SQL code. Prisma handled that for us!
...
The [SQLite Viewer VS Code extension](https://marketplace.visualstudio.com/items?itemName=qwtel.sqlite-viewer) is perfect for previewing your SQLite database directly within VS Code.
...
Bee Keeper Studio is another option. It's a stand alone application, but the free version will allow you to create, read, update, and delete data.
...
Instead of manually adding data to the database, we can create a seed file to programmatically add data for us. This is particularly helpful if you need to reset the database during development.
...
* **On line 7-14**, we're using raw SQL to delete the existing data from the database. This will allow us seed the database with fresh data.
...
* **On line 26**, we're logging a message to the console that the seeding is complete.
...
But, I'll also create seed files to test the application with various sets of data. This is much easier (and faster) than manually adding data.
