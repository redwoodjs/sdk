...
- On **lines 21-48**, we're setting up our middleware. This will run between every request and response cycle.
  ...
  - On **lines 27-36**, we're catching any errors. If there's an error or the status code is 401, we remove the session and redirect the user to the login page.
...
---
- **On line 17**, we're setting up our Turnstile site key. [We already set this up.](/tutorial/full-stack-app/create-app#setting-up-cloudflare-turnstile)
---
```

```tsx title="src/app/pages/user/Login.tsx" startLineNumber=20
const [username, setUsername] = useState("");
const [result, setResult] = useState("");
---
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=22
const [isPending, startTransition] = useTransition();
---
- **On line 22**, we're using React's `useTransition` hook to manage our loading state. (More on this later, but you can also find more within [the official React documentation](https://react.dev/reference/react/useTransition).)
---
```
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=25
const passkeyLogin = async () => {
  // 1. Get a challenge from the worker
  const options = await startPasskeyLogin();
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=42
  const passkeyRegister = async () => {
    // 1. Get a challenge from the worker
    const options = await startPasskeyRegistration(username);
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=65
const handlePerformPasskeyLogin = () => {
  startTransition(() => void passkeyLogin());
};
---
- **On lines 65**, the`handlePerformPasskeyLogin` function handles the transition and calls the `passkeyLogin` function.
---
```

```tsx title="src/app/pages/user/Login.tsx" startLineNumber=69
const handlePerformPasskeyRegister = () => {
  startTransition(() => void passkeyRegister());
};
---
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=77
<input
  type="text"
  value={username}
  onChange={(e) => setUsername(e.target.value)}
