...
Since we're using the [Pass Key Auth starter kit](https://github.com/redwoodjs/sdk/tree/main/starters/passkey-auth), it already has PassKey authentication setup.
...
```tsx title="src/worker.tsx" collapse={1-11}
...
setCommonHeaders(),
```
- On **line 19** we're setting up our common headers. This is a function inside of `app/src/headers.ts`. (Imported on line 5.) It creates the Security Policies, Transport Policy, Referrer Policy, and Permissions Policy.
...
```
- On **lines 20-47**, we're setting up our middleware.
  - First, we're setting up the database `setupDb(env);` We'll use this later, on line 41, to find the logged in user.
  ...
  - On **lines 26-38**, we're catching any errors. If the status code is 401, we remove the session and redirect the user to the login page.
  - On **lines 40-46**, we check the `AppContext` for the `session` object. If it has a `userId`, we connect to the database and get all the user data. The result gets saved into the `AppContext.user` object.

```tsx title="src/worker.tsx" startLineNumber=48
render(Document, [
```
- On **line 48**, we're setting up our document. This document contains our `html`, `head`, and `body` tags and wraps _all_ of our pages. **_RedwoodSDK only supports one document._**
...
```
- On **line 48-59**, we're setting up our home page route. Before we return our `Home` component (line 58), we use an **interrupter** to see if the user is logged in. This is a function that takes our `appContext` object and checks to see if the `user` object exists on our app context. If it doesn't, we redirect the user to the login page (lines 52-55). Otherwise, we return the `Home` component (line 58).
...
prefix("/user", userRoutes),
```
- On **line 60**, we're prefixing all of our auth routes with `/user`. Meaning, our `login` route will be `/user/login` and our `logout` route will be `/user/logout`.
...
```tsx title="src/app/pages/user/Login.tsx" collapse={2-16}
"use client";
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=18
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=23
const [isPending, startTransition] = useTransition();
---
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=24
const turnstile = useTurnstile(TURNSTILE_SITE_KEY);
---
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=26
const passkeyLogin = async () => {
  const options = await startPasskeyLogin();
  const login = await startAuthentication({ optionsJSON: options });
  const success = await finishPasskeyLogin(login);
...
- The functions `startPasskeyLogin`, `startAuthentication`, and `finishPasskeyLogin` are all defined in the `src/app/pages/user/functions.ts` file. You can find more information about these functions in the [Passkey Authentication docs](/core/authentication).
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=56
const handlePerformPasskeyLogin = () => {
  startTransition(() => void passkeyLogin());
};
---
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=60
const handlePerformPasskeyRegister = () => {
  startTransition(() => void passkeyRegister());
};
---
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=68
<input
  type="text"
  value={username}
  onChange={(e) => setUsername(e.target.value)}
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=80
{result && <div>{result}</div>}
---
...
When we installed ShadCN, it added a lot of variables to the bottom of our `styles.css` file. In CSS, order matters. So, let's move our `@theme` definition to the bottom of the file so that it reads as more important and will override any ShadCN default values.
...
```tsx title="src/app/pages/user/Login.tsx" {3-7} startLineNumber=35
return (
  <AuthLayout>
...
```tsx title="src/app/pages/user/Login.tsx" {1-3, 16} startLineNumber=45
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=57 {5}
...
```css title="src/styles.css" startLineNumber=152
...
```tsx title="src/app/pages/user/Login.tsx" "page-title" startLineNumber=47
...
```tsx title="src/app/pages/user/Login.tsx" "auth-form" startLineNumber=46 collapse={2-15}
...
```tsx title="src/app/pages/user/Login.tsx" collapse={6-18} startLineNumber=46 "{result && <div>{result}</div>}"
<div className="auth-form max-w-[400px] w-full mx-auto px-10">
  <h1 className="page-title text-center">Login</h1>
  <p className="py-6">Enter your username below to sign-in.</p>
...
We can also style this, using an `<Alert />` ShadCN component. We just need to import our component at the top and and wrap our `{result}`.
...
I know, it feels strange to see a red alert message when we were able to successfully login. But, really, if we were able to login successfully, we want to be redirected to the homepage. We can update the `passkeyLogin` function (starts on line 25):

```tsx title="src/app/pages/user/Login.tsx" {4} startLineNumber=30
if (!success) {
  setResult("Login failed");
} else {
...
```tsx title="src/app/shared/links.ts" {3-7} startLineNumber=3
export const link = defineLinks([
  "/",
  "/user/login",
  "/user/signup",
...
```tsx title="src/worker.tsx" collapse={2-30} startLineNumber=19 {44-45}
export default defineApp<AppContext>([
  setCommonHeaders(),
  async ({ env, appContext, request, headers }) => {
    await setupDb(env);
...
---
- **On line 62**, let's add a new `route` definition to our array for our privacy policy page. The `route` function takes two parameters. The first parameter is the path or the URL. The second parameter is the response. This could be [a standard HTML response object](https://developer.mozilla.org/en-US/docs/Web/API/Response) or JSX/TSX. For our quick solution, we're inlining JSX.
- **On line 63**, we're doing the exact same thing for our Terms page.
---
```
...
import { defineApp, ErrorResponse } from "@redwoodjs/sdk/worker";
import { index, render, prefix, route } from "@redwoodjs/sdk/router";
import { Document } from "@/app/Document";
import { Home } from "@/app/pages/Home";
import { setCommonHeaders } from "@/app/headers";
import { userRoutes } from "@/app/pages/user/routes";
...
```tsx title="src/app/pages/user/Signup.tsx" del={4, 8} ins={1,5,7} showLineNumbers=false
import { AuthLayout } from "@/app/layouts/AuthLayout";
...
return (
  <>
...
```tsx title="src/app/pages/user/Signup.tsx" showLineNumbers=false "/user/login" "Login"
import { link } from "@/app/shared/links";
...
<AuthLayout>
  <div className="absolute top-0 right-0 p-10">
...
```tsx title="src/app/pages/user/Signup.tsx" startLineNumber=54 "auth-form max-w-[400px] px-10"
<div className="auth-form max-w-[400px] w-full mx-auto px-10">
  ...
</div>
---
...
```tsx title="src/app/pages/user/Signup.tsx" startLineNumber=56
...
```tsx title="src/app/pages/user/Signup.tsx" showLineNumbers=false
import { Alert, AlertTitle } from "@/app/components/ui/alert";
import { AlertCircle } from "lucide-react";
...
{result && (
...
```tsx title="src/app/pages/user/Signup.tsx" "className="font-display w-full mb-6"" startLineNumber=75
...
```tsx title="src/app/pages/user/Signup.tsx" startLineNumber=79
<p>By clicking continue, you agree to our <a href={link('/legal/terms')}>Terms of Service</a> and <a href={link('/legal/privacy')}>Privacy Policy</a>.</p>
```
