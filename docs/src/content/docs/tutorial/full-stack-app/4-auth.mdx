...
```tsx title="src/components/ui/button.tsx" collapse={8-29} startLineNumber=9 "buttonVariants" "default:" "font-bold"
...
```tsx title="src/app/pages/user/Login.tsx" {14} collapse={2-10} startLineNumber=30
const passkeyLogin = async () => {
  // 1. Get a challenge from the worker
  const options = await startPasskeyLogin();
...
```tsx title="src/app/pages/user/Signup.tsx" startLineNumber=56 "auth-form max-w-[400px] px-10"
<AuthLayout>
  <div className="auth-form max-w-[400px] w-full mx-auto px-10">
    ...
    <div className="absolute top-0 right-0 p-10">
...
```tsx title="src/app/pages/user/Signup.tsx" showLineNumbers=false {1-2, 7-12}
import { Alert, AlertTitle } from "@/app/components/ui/alert";
import { AlertCircle } from "lucide-react";
...
<h1 className="page-title text-center">Create an Account</h1>
...
```tsx title="src/app/pages/user/Signup.tsx" "className="font-display w-full mb-6"" startLineNumber=83
