...
```tsx title="src/worker.tsx" startLineNumber=2
export type Context = {
  session: Session | null;
  user: User | null;
}
```
- On **lines 2-5**, we're setting up the types for our context object. Specifically, we're defining the `session` and `user` properties.
  - The `Session` type is defined in `src/session/durableObject.ts`. It contains a `userId`, `challenge`, and `createdAt`.
  - The `User` type comes from our `@prisma/client`. That's right! When Prisma generates our migration files, it also generates types.

```tsx title="src/worker.tsx" startLineNumber=8
setCommonHeaders(),
```
- On **line 8** we're setting up our common headers. This is a function inside of `app/src/headers.ts`. It creates the Security Policies, Transport Policy, Referrer Policy, and Permissions Policy.

```tsx title="src/worker.tsx" startLineNumber=9
async ({ env, ctx, request }) => {
  await setupDb(env);
  setupSessionStore(env);
  ctx.session = await sessions.load(request);
...
```tsx title="src/worker.tsx" startLineNumber=22
layout(Document, [
```
- On **line 22**, we're setting up our layout. This layout contains our `html`, `head`, and `body` tags and wraps _all_ of our pages. **_RedwoodSDK only supports one layout._**

```tsx title="src/worker.tsx" startLineNumber=23
index([
  ({ ctx }) => {
    if (!ctx.user) {
      return new Response(null, {
...
```tsx title="src/worker.tsx" startLineNumber=34
prefix("/user", authRoutes),
```
- On **line 34**, we're prefixing all of our auth routes with `/user`. Meaning, our `login` route will be `/user/login` and our `logout` route will be `/user/logout`. One of the cool things about the RedwoodSDK router is the parameter is defined through an array. This means, we can define our route information in a separate file and then import it into our `worker.tsx` file.

Here, we're co-locating the file that defines our auth routes with our auth pages.

Let's take a look at `authRoutes`. This is coming from `src > app > pages > auth > routes.tsx`.
...
- `/logout` - This is our logout route, but instead of returning JSX, we're removing the session (line 10) and redirecting the user to the home page (line 11). Then, returning the response (line 13-16).
...
```tsx title="src/app/pages/auth/routes.tsx" collapse={1-2, 4-7, 11-21}
...
<Aside type="note" title="Page Naming Conventions">
You'll notice, we're appending `Page` to all our page component file names (`HomePage.tsx`, `LoginPage.tsx`, `SignupPage.tsx`). This is personal preference, but a good convention for identifying page components, especially if you're co-locating components and pages.
</Aside>

Now, let's move over some of the functionality from our `LoginPage.tsx` to our `SignupPage.tsx`. But first, let's make sure we understand what's happening in our `LoginPage.tsx`.

<details>
<summary>Full `LoginPage.tsx` code</summary>

```tsx title="src/app/pages/auth/LoginPage.tsx" collapse={2-15}
"use client";
...
const TURNSTILE_SITE_KEY = "0xXXXXXXXXXXXXXXXXXXXXXX";
```
- **On line 18**, we're setting up our Turnstile site key. [We already set this up.](/tutorial/full-stack-app/create-app#setting-up-cloudflare-turnstile)

```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=21
const [username, setUsername] = useState("");
const [result, setResult] = useState("");
```
- **On lines 21-22**, we're setting a couple pieces of state.
  - `username` is tied to our username input value.
  - `result` contains our result message.

```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=23
const [isPending, startTransition] = useTransition();
```
- **On line 23**, we're using React's `useTransition` hook to manage our loading state. (More on this later, but you can also find more within [the official React documentation](https://react.dev/reference/react/useTransition).)

```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=24
const turnstile = useTurnstile(TURNSTILE_SITE_KEY);
```
- **On line 24**, the `useTurnstile` hook gets our Turnstile instance. This hook is coming from RedwoodSDK and is used to handle the challenge and verify our Turnstile token.

```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=26
const passkeyLogin = async () => {
  const options = await startPasskeyLogin();
  const login = await startAuthentication({ optionsJSON: options });
  const success = await finishPasskeyLogin(login);
...
```
- **On line 26**, we're defining our `passkeyLogin` function.
  - The functions `startPasskeyLogin`, `startAuthentication`, and `finishPasskeyLogin` are all defined in the `src/app/pages/auth/functions.ts` file. You can find more information about these functions in the [Passkey Authentication docs](/docs/auth/passkeys).
  - Here, we start the Passkey login, authentication, and then determine whether it was successful or not.
  - If it wasn't successful, we set the result message to "Login failed".
  - Otherwise, we set the result message to "Login successful!".

```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=38
const passkeyRegister = async () => {
  const options = await startPasskeyRegistration(username);
  const registration = await startRegistration({ optionsJSON: options });
  const turnstileToken = await turnstile.challenge();
...
```
- **On lines 38-54**, we're defining our `passkeyRegister` function. This is similar the `passkeyLogin` function, but this handles the registration process.
  - The functions `startPasskeyRegistration`, `startRegistration`, and `finishPasskeyRegistration` are all defined in the `src/app/pages/auth/functions.ts` file. You can find more information about these functions in the [Passkey Authentication docs](/docs/auth/passkeys).
...
```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=56
const handlePerformPasskeyLogin = () => {
  startTransition(() => void passkeyLogin());
};
```
- **On lines 56-58**, the`handlePerformPasskeyLogin` function handles the transition and calls the `passkeyLogin` function.

```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=60
const handlePerformPasskeyRegister = () => {
  startTransition(() => void passkeyRegister());
};
```
- **On lines 60-62**, the`handlePerformPasskeyRegister` function handles the transition and calls the `passkeyRegister` function.

```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=68
<input
  type="text"
  value={username}
  onChange={(e) => setUsername(e.target.value)}
...
/>
```
- **On lines 68-70**, we're rendering an input field. This is tied to our `username` state.
...
```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=74
<Button onClick={handlePerformPasskeyLogin} disabled={isPending}>
  {isPending ? <>...</> : "Login with passkey"}
</Button>
<Button onClick={handlePerformPasskeyRegister} disabled={isPending}>
...
```tsx title="src/app/pages/auth/LoginPage.tsx" startLineNumber=80
{result && <div>{result}</div>}
```
...
Let's start by copying all the contents from our `LoginPage.tsx` and pasting it into our `SignupPage.tsx`. Be sure that the component is still named `SignupPage`.

```tsx title="src/app/pages/auth/SignupPage.tsx" startLineNumber=20
export function SignupPage() {
```

Let's remove all the pieces that aren't related to registering.
- We don't need the `passkeyLogin` function (lines 26-30)
...
<details>
<summary>Cleaned up `SignupPage.tsx` code</summary>
...
Now, let's do the opposite for our `LoginPage.tsx`.
...
<details>
<summary>Cleaned up `LoginPage.tsx` code</summary>
...
Inside our `LoginPage` component let's replace our `main` tag with our `AuthLayout` component.

```tsx title="src/app/pages/auth/LoginPage.tsx" {2,15} startLineNumber=35
return (
  <AuthLayout>
  <h1 className="text-4xl font-bold text-red-500">YOLO</h1>
    <div ref={turnstile.ref} />
...
For the column on the left, we have a repeating background image. You can export this image from Figma yourself, or [grab all the image assets for this project here](https://drive.google.com/drive/folders/1VYvgh6nvxvxamHOomfwEGG9DYVbvfOvS?usp=drive_link).
...
While we're here, let's also grab the logo. I'm only going to grab the owl. We can make "Apply Wize" text, styled with CSS. This will give us a little more control over the display (horizontal or stacked) without having to use two separate images. You can export the owl as an SVG (or [download it directly](https://drive.google.com/drive/folders/1VYvgh6nvxvxamHOomfwEGG9DYVbvfOvS?usp=drive_link)).
...
---
Within Tailwind v4, you can add custom utility functions with the `@utility` directive. _NOTE: `center` does not have a `.` in front, like a class would._
---
```
...
```tsx title="src/app/layouts/AuthLayout.tsx" startLineNumber=7
<img src="/images/logo.svg" alt="Apply Wize" />
```

Now, let's style the Apply Wize text.

```tsx title="src/app/layouts/AuthLayout.tsx" {11} startLineNumber=8
<div className="text-5xl text-white font-bold">
  Apply Wize
</div>
---
...
```tsx title="src/app/layouts/AuthLayout.tsx" startLineNumber=12
...
Our parent `div` doesn't have any positioning applied (by default it's `static`). So, then, it looks to the page. And, yes, our quote is positioned to the bottom left corner of the page, with `40px` of padding.
...
Now, we can jump over to our `LoginPage` component and style it.
...
```tsx title="src/app/pages/auth/LoginPage.tsx" {3-7} startLineNumber=35
return (
  <AuthLayout>
...
```tsx title="src/app/pages/auth/LoginPage.tsx" collapse={4-15} startLineNumber=43
...
---
- On our wrapping `div` I don't want the content to be more than `400px` wide with `max-w-[400px]`
- On smaller screens, I want to ensure there's some horizontal padding with `px-10`
- Our `h1` should be centered `text-center`
- I also want to add some vertical spacing with `py-6`
---
```

I want to some more styling, but I want to be able to reuse these styles on our Signup page, so let's stick these inside our `styles.css` file.
...
Did these `@layer`s give you de ja vu. Feels similar to Tailwind v3? It's the exact same concept! But, because these layers are no longer established in Tailwind, we have to define the order ourselves: `@layer base, components, page;`
...
Back in our `LoginPage` component, let's add the class of `page-title` to our `h1`:

```tsx title="src/app/pages/auth/LoginPage.tsx" "page-title" startLineNumber=44
...
```tsx title="src/app/pages/auth/LoginPage.tsx" "auth-form" startLineNumber=43 collapse={2-15}
...
```css title="src/styles.css" showLineNumbers=false collapse={2-5}
@layer base {
  h1, h2, h3 {
    @apply font-display font-bold;
  }
...
For the button, we're using the ShadCN button component. Let's crack it open:

```tsx title="src/components/ui/button.tsx" collapse={8-29} startLineNumber=7 "buttonVariants" "default:" "font-bold"
...
- The first variant is the `default` variant and has a class of `bg-primary`. This is why our button is using our yellow, primary color out of the box. We've already set `--color-primary` within our `styles.css` file.
---
```
...
```tsx title="src/styles.css" startLineNumber=55 "className="font-display w-full mb-6"
...
```tsx title="src/app/pages/auth/LoginPage.tsx" collapse={6-18}
<div className="auth-form max-w-[400px] px-10">
  <h1 className="page-title text-center">Login</h1>
  <p className="py-6">Enter your username below to sign-in.</p>
...
We can also style this, using an `<Alert />` ShadCN component.

```tsx title="src/app/pages/auth/LoginPage.tsx" showLineNumbers=false
...
---
- `Alert`, `AlertCircle`, and `AlertTitle` are all from the ShadCN `Alert` component. [You can find the full documentation here.](https://ui.shadcn.com/docs/components/alert)
...
I know, it feels strange to see a red alert message when we were able to successfully login. But, really, if we were able to login successfully, we want to be redirected to the homepage. We can update the `passkeyLogin` function:

```tsx title="src/app/pages/auth/LoginPage.tsx" {4} startLineNumber=27
if (!success) {
  setResult("Login failed");
} else {
...
```tsx title="src/app/shared/links.ts" {3-6} startLineNumber=3
export const link = defineLinks([
  "/",
  "/user/login",
  "/user/signup",
...
Now, let's go back to our `LoginPage` component and update our **Register** link.
...
```tsx title="src/app/pages/auth/LoginPage.tsx" "link" startLineNumber=4
<a href={link('/legal/terms')} className="font-display font-bold text-black text-sm underline underline-offset-8 hover:decoration-primary">
  Terms of Service
</a>
```
...
```tsx title="src/worker.tsx" collapse={2-27} startLineNumber=18 {29-30}
export default defineApp<Context>([
  setCommonHeaders(),
  async ({ env, ctx, request }) => {
    await setupDb(env);
...
---
- **On line 46**, let's add a new `route` definition to our array for our privacy policy page. The `route` function takes two parameters. The first parameter is the path or the URL. The second parameter is the response. This could be [a standard HTML response object](https://developer.mozilla.org/en-US/docs/Web/API/Response) or JSX/TSX. For our quick solution, we're inlining JSX.
- **On line 47**, we're doing the exact same thing for our Terms page.
---
```
...
<details>
<summary>Final code for our `LoginPage.tsx`</summary>
...
@utility center { @apply flex items-center justify-center; }
...
<details>
<summary>Final code for our `button.tsx`</summary>
...
<details>
<summary>Final code for our `links.ts`</summary>
...
```tsx title="src/app/pages/auth/SignupPage.tsx" del={4, 8} ins={1,5,7} showLineNumbers=false
import { AuthLayout } from "@/app/layouts/AuthLayout";
...
return (
  <>
...
```tsx title="src/app/pages/auth/SignupPage.tsx" showLineNumbers=false "/user/login" "Login"
import { link } from "@/app/shared/links";
...
<AuthLayout>
  <div className="absolute top-0 right-0 p-10">
...
```tsx title="src/app/pages/auth/SignupPage.tsx" startLineNumber=55 "auth-form max-w-[400px] px-10"
<div className="auth-form max-w-[400px] px-10">
  ...
</div>
---
...
- We've set the maximum width of our form to `400px` and added `40px` padding to the left and right with `px-10.
---
```

We can get rid of our red YOLO heading at the top and replace it with:
```tsx title="src/app/pages/auth/SignupPage.tsx" startLineNumber=56
...
```tsx title="src/app/pages/auth/SignupPage.tsx" showLineNumbers=false
import { Alert, AlertTitle } from "@/app/components/ui/alert";
import { AlertCircle } from "lucide-react";
...
{result && (
...
```tsx title="src/app/pages/auth/SignupPage.tsx" "className="font-display w-full mb-6"" startLineNumber=75
...
```tsx title="src/app/pages/auth/SignupPage.tsx" startLineNumber=79
<p>By clicking continue, you agree to our <a href={link('/legal/terms')}>Terms of Service</a> and <a href={link('/legal/privacy')}>Privacy Policy</a>.</p>
```
...
<details>
<summary>Final code for our `SignupPage.tsx`</summary>
