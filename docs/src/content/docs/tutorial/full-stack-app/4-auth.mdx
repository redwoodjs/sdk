...
Think of it like a mailbox system: You give the website a special mailbox (the public key), that only your unique key can open. When you want to log in, the website puts a secret message in your mailbox. Only you have the key to open it and read teh message, which proves you're the real account owner.
...
- On **lines 13-16**, we're setting up the types for our application's context object. Specifically, we're defining the `session` and `user` properties.
  ...
  - The `User` type comes from our `@generated/prisma`. (Imported on line 9.) That's right! When Prisma generates our migration files, it also generates types.
...
To wrap our `Login` component with the new `AuthLayout` component, we simply import the layout and AuthLayout, and then wrap the `user` routes with the AuthLayout:

```tsx title="src/worker.tsx" ins={2, 13, 64} collapse={3-12, 14-62}
import { defineApp, ErrorResponse } from "rwsdk/worker";
import { ..., layout } from "rwsdk/router";
import { Document } from "@/app/Document";
import { Home } from "@/app/pages/Home";
import { setCommonHeaders } from "@/app/headers";
import { userRoutes } from "@/app/pages/user/routes";
...
This mean any route inside the `UserRoutes` will lbe wrapped by the new AuthLayout. 
...
Because we allready wraped our `UserRoutes` with the AuthLayout, all the pages are wrapped with the `AuthLayout` component.

Let's add our Login link to the Signup page:

```tsx title="src/app/pages/user/Signup.tsx" showLineNumbers=false "/user/login" "Login" ins={1, 3-7}
import { link } from "@/app/shared/links";
...
  <div className="absolute top-0 right-0 p-10">
...
```tsx title="src/app/pages/user/Signup.tsx" startLineNumber=56 "auth-form max-w-[400px] px-10" ins={1,8}
  <div className="auth-form max-w-[400px] w-full mx-auto px-10">
    <div className="absolute top-0 right-0 p-10">
      <a href={link('/user/login')} className="font-display font-bold text-black text-sm underline underline-offset-8 hover:decoration-primary">
        Login
