...
```
- On **lines 14-17**, we're setting up the types for our context object. Specifically, we're defining the `session` and `user` properties.
...
setCommonHeaders(),
```
- On **line 20** we're setting up our common headers. This is a function inside of `app/src/headers.ts`. It creates the Security Policies, Transport Policy, Referrer Policy, and Permissions Policy.
...
```
- On **lines 21-48**, we're setting up our middleware.
  ...
  - On **lines 27-39**, we're catching any errors. If the status code is 401, we remove the session and redirect the user to the login page.
  - On **lines 41-46**, we check the `ctx` for the `session` object. If it has a `userId`, we connect to the database and get all the user data. The result gets saved into the `ctx.user` object.

{/* TODO: Layouts are going to become documents */}

```tsx title="src/worker.tsx" startLineNumber=49
layout(Document, [
```
- On **line 49**, we're setting up our layout. This layout contains our `html`, `head`, and `body` tags and wraps _all_ of our pages. **_RedwoodSDK only supports one layout._**

```tsx title="src/worker.tsx" startLineNumber=48
layout(Document, [
    index([
      ({ ctx }) => {
        if (!ctx.user) {
...
```
- On **line 48-59**, we're setting up our home page route. Before we return our `Home` component (line 58), we use an **interrupter** to see if the user is logged in. This is a function that takes our `ctx` object and checks to see if the `user` object exists on our context. If it doesn't, we redirect the user to the login page. Otherwise, we return the `Home` component.
...
Inside, our `src/app/pages/auth` directory, create a new file called `Signup.tsx`. Let's stub out a simple function, just to make sure it's working:

```tsx title="src/app/pages/auth/Signup.tsx"
export const Signup = () => {
...
<details>
<summary>Full `Login.tsx` code</summary>

```tsx title="src/app/pages/auth/Login.tsx" collapse={2-16}
"use client";
...
```tsx title="src/app/pages/auth/Login.tsx"
"use client";
---
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=21
const [username, setUsername] = useState("");
const [result, setResult] = useState("");
---
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=23
const [isPending, startTransition] = useTransition();
---
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=24
const turnstile = useTurnstile(TURNSTILE_SITE_KEY);
---
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=26
const passkeyLogin = async () => {
  const options = await startPasskeyLogin();
  const login = await startAuthentication({ optionsJSON: options });
  const success = await finishPasskeyLogin(login);
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=38
const passkeyRegister = async () => {
  const options = await startPasskeyRegistration(username);
  const registration = await startRegistration({ optionsJSON: options });
  const turnstileToken = await turnstile.challenge();
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=56
const handlePerformPasskeyLogin = () => {
  startTransition(() => void passkeyLogin());
};
---
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=60
const handlePerformPasskeyRegister = () => {
  startTransition(() => void passkeyRegister());
};
---
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=68
<input
  type="text"
  value={username}
  onChange={(e) => setUsername(e.target.value)}
...
/>
---
- **On lines 68-73**, we're rendering an input field. This is tied to our `username` state.
---
```
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=74
<Button onClick={handlePerformPasskeyLogin} disabled={isPending}>
  {isPending ? <>...</> : "Login with passkey"}
</Button>
<Button onClick={handlePerformPasskeyRegister} disabled={isPending}>
...
```tsx title="src/app/pages/auth/Login.tsx" startLineNumber=80
{result && <div>{result}</div>}
---
...
```tsx title="src/app/pages/auth/Signup.tsx" startLineNumber=20
...
```tsx title="src/app/pages/auth/Login.tsx" {2,15} startLineNumber=35
return (
  <AuthLayout>
  <h1 className="text-4xl font-bold text-red-500">YOLO</h1>
    <div ref={turnstile.ref} />
...
```tsx title="src/app/pages/auth/Login.tsx" {3-7} startLineNumber=35
return (
  <AuthLayout>
...
```tsx title="src/app/pages/auth/Login.tsx" collapse={4-15} startLineNumber=43
...
```tsx title="src/app/pages/auth/Login.tsx" "page-title" startLineNumber=44
...
```tsx title="src/app/pages/auth/Login.tsx" "auth-form" startLineNumber=43 collapse={2-15}
...
```tsx title="src/app/pages/auth/Login.tsx" collapse={6-18}
<div className="auth-form max-w-[400px] px-10">
  <h1 className="page-title text-center">Login</h1>
  <p className="py-6">Enter your username below to sign-in.</p>
...
```tsx title="src/app/pages/auth/Login.tsx" showLineNumbers=false
...
```tsx title="src/app/pages/auth/Login.tsx" {4} startLineNumber=27
if (!success) {
  setResult("Login failed");
} else {
...
```tsx title="src/app/pages/auth/Login.tsx" "link" startLineNumber=4
...
<details>
<summary>Final code for our `Login.tsx`</summary>
...
```tsx title="src/app/pages/auth/Signup.tsx" del={4, 8} ins={1,5,7} showLineNumbers=false
import { AuthLayout } from "@/app/layouts/AuthLayout";
...
return (
  <>
...
```tsx title="src/app/pages/auth/Signup.tsx" showLineNumbers=false "/user/login" "Login"
import { link } from "@/app/shared/links";
...
<AuthLayout>
  <div className="absolute top-0 right-0 p-10">
...
```tsx title="src/app/pages/auth/Signup.tsx" startLineNumber=55 "auth-form max-w-[400px] px-10"
<div className="auth-form max-w-[400px] px-10">
  ...
</div>
---
...
```tsx title="src/app/pages/auth/Signup.tsx" startLineNumber=56
...
```tsx title="src/app/pages/auth/Signup.tsx" showLineNumbers=false
import { Alert, AlertTitle } from "@/app/components/ui/alert";
import { AlertCircle } from "lucide-react";
...
{result && (
...
```tsx title="src/app/pages/auth/Signup.tsx" "className="font-display w-full mb-6"" startLineNumber=75
...
```tsx title="src/app/pages/auth/Signup.tsx" startLineNumber=79
<p>By clicking continue, you agree to our <a href={link('/legal/terms')}>Terms of Service</a> and <a href={link('/legal/privacy')}>Privacy Policy</a>.</p>
```
...
<details>
<summary>Final code for our `Signup.tsx`</summary>
