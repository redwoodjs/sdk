...
<Aside type="note" title="What is Passkey Authentication?">
...
<Aside type="note" title="What is a Cloudflare Worker?">
...
In traditional web development, you might have a separate server running somewhere (like on AWS or your own computer) that handles requests when someone visits your website. A Cloudflare Worker does the same job, but it runs on Cloudflare's global network instead.
The key benefits for our tutorial:
...
```tsx title="src/worker.tsx" startLineNumber=13
export type AppContext = {
  session: Session | null;
  user: User | null;
};
```
- On **lines 13-16**, we're setting up the types for our application's context object. Specifically, we're defining the `session` and `user` properties.
  ...
  - The `User` type comes from our `@generated/prisma`. (Imported on line 9.)

```tsx title="src/worker.tsx" startLineNumber=19
setCommonHeaders(),
```
...
```
- On **lines 20-47**, we're setting up our middleware. This will run between every request and response cycle.
  - First, we're setting up the database `setupDb(env);` We'll use this later, on line 42-44, to find the logged in user.
  ...
  - On **lines 26-35**, we're catching unauthorized errors. If there's an error and the status code is 401, we remove the session and redirect the user to the login page.
  - On **lines 40-46**, we check the `ctx` for the `session` object. If it has a `userId`, we connect to the database and get all the user data. The result gets saved into the `ctx.user` object.

```tsx title="src/worker.tsx" startLineNumber=48
render(Document, [
```
- On **line 48**, we're setting up our document. This document is referencing our `src/app/Document.tsx` file. It contains our `html`, `head`, and `body` tags and wraps _all_ of our pages.
...
render(Document, [
  route("/", () => new Response("Hello, World!")),
```
- On **line 49**, we're setting up our home page route. Here, we're just returning a standard response.

The router highlights the request-response lifecycle. The first parameter is the route we're requesting and the second parameter is the response. This can be a standard response, or it can be a React component. On line 49, we're returning a standard response that says `Hello World!`.

We can change that to return the `Home` page component by changing the second parameter:
...
Here, we're using an **interrupter** to see if the user is logged in. This is a function that takes our `ctx` object and checks to see if the `user` object exists on it. If it doesn't, we redirect the user to the login page (lines 53-56). Otherwise, we return the `Home` component (line 59).
...
prefix("/user", userRoutes),
```
- On **line 61**, we're prefixing all of our auth routes with `/user`. Meaning, our `login` route will be `/user/login` and our `logout` route will be `/user/logout`.
...
Now, moving down to our `return` statement, inside:
...
/>
---
- We're rendering an input field. This is tied to our `username` `useState` (line 17).
---
```
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=78
...
```tsx title="src/app/pages/user/Signup.tsx" startLineNumber=16
...
```tsx title="src/app/pages/Signup.tsx" startLineNumber=26
if (!success) {
  setResult("Registration failed");
} else {
...
```tsx title="src/worker.tsx" startLineNumber=50 {3-7}
route("/protected", [
  ({ ctx }) => {
    if (!ctx.user) {
      return new Response(null, {
...
---
**On line 52**, we're checking to see if the `user` object exists on the `ctx` object. If it doesn't, we redirect the user to the `/user/login` page.
---
```
...
![](./images/move-theme-definition.png)

![](./images/to-above-layer-base.png)
...
export { AuthLayout };
---
Our layout component types are imported from `rwsdk/router`. This includes an object containing `children` and `requestInfo`. It's worth noting that the `requestInfo` object is only passed to server components ([additional documentation](/guides/frontend/layouts/#layout-props)).
...
To wrap all our `/user` pages with the new `AuthLayout` component, let's go back to our `worker.tsx` file.

At the top of our file, import the `layout` function from `rwsdk/router`:

```tsx title="src/worker.tsx" showLineNumbers=false ins={2} del={1} "layout"
import { route, render, prefix } from "rwsdk/router";
...
```tsx title="src/worker.tsx" startLineNumber=61 "layout(AuthLayout, [" "])"
...
We'll also need to import the `AuthLayout` component we just referenced:

```tsx title="src/worker.tsx" showLineNumbers=false
import { AuthLayout } from "@/app/layouts/AuthLayout";
```

<Aside type="note" title="Layout Function">
You don't have to use the `prefix` function when using the `layout` function. You can simply pass in an array of routes. For example:

```tsx title="src/worker.tsx" showLineNumbers=false
layout(AuthLayout, [
  route("/user/login", Login),
  route("/user/signup", Signup),
]),
```
...
```tsx title="src/worker.tsx" showLineNumbers=false
layout(AuthLayout, [route("/user/login", Login)]),
```
</Aside>

<Aside type="note" title="Nested Layouts">
You can also nest layouts. For example:

```tsx title="src/worker.tsx" showLineNumbers=false
layout(BaseLayout, [layout(AuthLayout, [
  route("/user/login", Login),
  route("/user/signup", Signup),
])]),
```
...
```tsx title="src/app/layouts/AuthLayout.tsx" ins={3, 5} startLineNumber=4
return (
...
```tsx title="src/app/layouts/AuthLayout.tsx" startLineNumber=4 ins={4,5}
return (
  <div className="bg-bg min-h-screen min-w-screen p-12">
    <div className="grid grid-cols-2 min-h-[calc(100vh-96px)] rounded-xl border-2 border-[#D6D5C5]">
      <div></div>
...
```css title="src/styles.css" startLineNumber=134
@utility center {
...
```tsx title="src/app/layouts/AuthLayout.tsx" "center" startLineNumber=4
  return (
    <div className="bg-bg min-h-screen min-w-screen p-12">
      <div className="grid grid-cols-2 min-h-[calc(100vh-96px)] rounded-xl border-2 border-[#D6D5C5]">
...
```tsx title="src/app/layouts/AuthLayout.tsx" {5,8,11} startLineNumber=4
return (
  <div className="bg-bg min-h-screen min-w-screen p-12">
    <div className="grid grid-cols-2 min-h-[calc(100vh-96px)] rounded-xl border-2 border-[#D6D5C5]">
      <div className="center bg-[url('/images/bg.png')] bg-repeat rounded-l-xl">
...
```tsx title="src/app/layouts/AuthLayout.tsx" startLineNumber=9 "className="mx-auto""
<img src="/images/logo.svg" alt="Apply Wize" className="mx-auto" />
```

Now, let's style the Apply Wize text.

```tsx title="src/app/layouts/AuthLayout.tsx" startLineNumber=10 "className="text-5xl text-white font-bold""
...
```tsx title="src/app/layouts/AuthLayout.tsx" {6} startLineNumber=7
<div className="center bg-[url('/images/bg.png')] bg-repeat rounded-l-xl">
  <div>
    <img src="/images/logo.svg" alt="Apply Wize" className="mx-auto" />
    <div className="text-5xl text-white font-bold">Apply Wize</div>
...
```tsx title="src/app/layouts/AuthLayout.tsx" "relative" startLineNumber=7
...
```tsx title="src/app/layouts/AuthLayout.tsx" startLineNumber=16 "bg-white rounded-r-xl" "className="w-full""
...
First we can delete our `h1`, **YOLO** text.

```tsx title="src/app/pages/user/Login.tsx" startLineNumber=36 del={1}
<h1 className="text-4xl font-bold text-red-500">YOLO</h1>
```

Let's also remove the `bg-bg` class from our wrapping `main` tag.

```tsx title="src/app/pages/user/Login.tsx" startLineNumber=35 del="className="bg-bg""
<main className="bg-bg">
```
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=34 ins={3-7}
return (
  <main>
...
```tsx title="src/app/pages/user/Login.tsx" {1-3, 15} startLineNumber=44
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=54 {5}
...
```css title="src/styles.css" ins={1,12-16} startLineNumber=138
...
```css title="src/styles.css" showLineNumbers=false ins={3-7}
@layer base {
  ...
  h1,
  h2,
...
```css title="src/styles.css" startLineNumber=155 ins={2-4}
@layer components {
  .page-title {
    @apply text-3xl
  }
...
```tsx title="src/app/pages/user/Login.tsx" "page-title" startLineNumber=45
...
```tsx title="src/app/pages/user/Login.tsx" "auth-form" startLineNumber=44
...
```css title="src/styles.css" showLineNumbers=false ins={3-5}
@layer base {
  ...
  input[type="text"] {
...
```tsx title="src/components/ui/button.tsx" collapse={8-29} startLineNumber=7 ins="font-bold" del="font-medium"
...
```tsx title="src/app/components/ui/button.tsx" startLineNumber=11 ins="text-black" del="text-primary-foreground"
variant: {
  default: "bg-primary text-primary-foreground text-black shadow-xs hover:bg-primary/90",
```
...
```tsx title="src/app/pages/user/Login.tsx" startLineNumber=54 ins="className="font-display w-full mb-6""
<Button
...
```tsx title="src/app/pages/user/Login.tsx" collapse={6-11} del={19} ins={4} showLineNumbers=false
<div className="auth-form max-w-[400px] w-full mx-auto px-10">
  <h1 className="page-title text-center">Login</h1>
  <p className="py-6">Enter your username below to sign-in.</p>
  {result && <div>{result}</div>}
...
```tsx title="src/app/pages/user/Login.tsx" del={14} ins={15} collapse={2-10} startLineNumber=15
const passkeyLogin = async () => {
  // 1. Get a challenge from the worker
  const options = await startPasskeyLogin();
...
```tsx title="src/app/shared/links.ts" ins={4, 6-7} startLineNumber=3
...
<Aside type="caution" title="Protected Page Route">
...
```tsx title="src/app/pages/user/Login.tsx" showLineNumbers=false ins={1} ins="{link("/user/signup")}"
...
Let's start by removing our YOLO heading and the `bg-bg` class on our wrapping `main` tag:

```tsx title="src/app/pages/user/Signup.tsx" startLineNumber=38 del={3} del="className="bg-bg""
return (
    <main className="bg-bg">
      <h1 className="text-4xl font-bold text-red-500">YOLO</h1>
```

Now, let's add a login link at the top of our signup page:

```tsx title="src/app/pages/user/Signup.tsx" ins={1, 5-12} showLineNumbers=false
import { link } from "@/app/shared/links";
...
  return (
    <main>
...
Next, let's wrap all of our content with a `div`:

```tsx title="src/app/pages/user/Signup.tsx" "auth-form max-w-[400px] px-10" ins={3, 22} collapse={4-21} startLineNumber=38
return (
    <main>
      <div className="auth-form max-w-[400px] w-full mx-auto px-10">
        <div className="absolute top-0 right-0 p-10">
...
Before our `input` tag, let's add a heading and a paragraph:

```tsx title="src/app/pages/user/Signup.tsx" collapse={3-8} startLineNumber=49 ins={1-2}
...
```tsx title="src/app/pages/user/Signup.tsx" ins={1-2, 6-11} del={21} collapse={12-20}
import { Alert, AlertTitle } from "@/app/components/ui/alert";
import { AlertCircle } from "lucide-react";
...
<h1 className="page-title text-center">Create an Account</h1>
...
```tsx title="src/app/pages/user/Signup.tsx" ins="className="font-display w-full mb-6"" startLineNumber=65
<Button
