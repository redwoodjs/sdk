...
...
prefix("/applications", [route("/", List)]),
---
...
This would get cumbersome (and annoying) if we have to do this for every.single.route we wanted to protect. Let's abstract this code into a reusable function. Right below our `AppContext` type, let's add a new function:
...
---
We're passing in the `ctx` that we automatically get with each request. Then, we're checking to see if the `user` exists on the context object. If it doesn't, we're returning a response that redirects the user to the login page.
---
```

Then, we can update our `/applications` prefix to use `isAuthenticated`:

```tsx title="src/worker.tsx" startLineNumber=75
prefix("/applications", [isAuthenticated, route("/", List)]),
```

Now, not only is our `/applications` route protected, but any route prefixed with `/application" is protected, too.

<Aside type="note" title="Protected Routes">
  The beauty of the RWSDK route syntax is that since we're using arrays, it's easy to control when the Middleware and/or Interrupters run.

  In our code, we're applying our `isAuthenticated` function to any route prefixed with `/applications`, but we could just as easily apply it to a single route:
...
While we're cleaning up our `worker.tsx` file, let's update the `/` and `/protected` routes:

```tsx title="src/worker.tsx" del={1-2,4-11} ins={3,12} showLineNumbers=false
...
---
- First, we can delete the existing home page `/` route.
- Then we can change the path for the `/protected` route to `/`.
- Finally, we can remove our authentication function and use the `isAuthenticated` function instead.
---
```

The updated code looks much cleaner:

```tsx title="src/worker.tsx" startLineNumber={60}
route("/", [isAuthenticated, Home]),
```

I want to take this a step further and show you another option. With the homepage route, you can also use the `index` function. This works very similar to the `route` function, but it assumes the path is `/`, so it only takes one argument: the response.
...
[Earlier, I mentioned that sometimes I'll create multiple seed files with various purposes.](/tutorial/full-stack-app/database-setup#seed-the-database). This is a perfect opportunity to create a separate file specifically for adding job applications to our database.
...
```tsx title="src/scripts/applicationSeed.ts" ins={7-11} startLineNumber=4
export default defineScript(async ({ env }) => {
  setupDb(env);
...
```tsx title="src/scripts/applicationSeed.ts" ins={8-12} startLineNumber=4
export default defineScript(async ({ env }) => {
  setupDb(env);
...
```tsx title="src/scripts/applicationSeed.ts" ins={8-20} startLineNumber=4
export default defineScript(async ({ env }) => {
  setupDb(env);
...
```json title="package.json" showLineNumbers=false ins={5}
...
  "scripts": {
    ...
...
```tsx title="src/app/pages/applications/List.tsx" ins="async" ins={1,4, 8}
import { db } from "@/db";
...
```tsx title="src/app/layouts/AuthLayout.tsx" {3-4} startLineNumber=3
...
```tsx title="src/app/layouts/AuthLayout.tsx" ins="page-wrapper" ins="page" startLineNumber=3
const AuthLayout = ({ children }: { children: React.ReactNode }) => {
  return (
...
```tsx title="src/app/layouts/InteriorLayout.tsx" ins="page-wrapper" ins="page bg-white" startLineNumber=3
...
```tsx title="src/app/worker.tsx" startLineNumber=65 ins={1,3-4}
layout(
  InteriorLayout,
  prefix("/applications", [isAuthenticated, route("/", List)])
),
...
```tsx title="src/app/components/Header.tsx" collapse={3-6} ins={7-11} startLineNumber=6
{/* left side */}
<div>
  <a href={link("/")}>
    <img src="/images/logo.svg" alt="Apply Wize" />
...
```tsx title="src/app/components/Header.tsx" ins={3-13} del={2,14} startLineNumber=21
{/* right side */}
<div>
<nav>
  <ul>
...
```tsx title="src/app/components/Header.tsx" showLineNumbers=false ins="AvatarImage" ins={8}
...
```tsx title="src/app/components/ui/avatar.tsx" startLineNumber=1 ins=""use client""
"use client"
...
```css title="src/styles.css" collapse={2-17} startLineNumber=116 ins={18}
@theme {
...
```tsx title="src/app/layouts/Header.tsx" startLineNumber=6 del="px-10" ins="px-page-side"
...
```tsx title="src/app/layouts/Header.tsx" startLineNumber=7 collapse={3-11} ins="className="flex items-center gap-8""
{/* left side */}
...
```tsx title="src/app/components/Header.tsx" startLineNumber=9 ins={3}
<a
  href={link("/")}
...
```tsx title="src/app/components/Header.tsx" startLineNumber=10 ins="className="pt-5 -mb-3""
...
```tsx title="src/app/components/Header.tsx" startLineNumber=25 ins="className="flex items-center gap-7""
{/* right side */}
<nav>
...
```css title="src/styles.css" collapse={2-18} startLineNumber=142 ins={19-21}
@layer base {
  * {
...
```tsx title="src/app/layouts/InteriorLayout.tsx" ins={2, 8}
import { type LayoutProps } from "rwsdk/router";
...
```tsx title="src/app/pages/applications/List.tsx" ins={1,8-16,18}
...
```tsx title="src/app/components/ui/button.tsx" ins={1}
"use client";
```
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=12 ins="className="px-page-side flex justify-between items-center"" ins="className="page-title""
...
```tsx title="src/app/pages/applications/List.tsx" collapse={2-17, 20-24} ins={1, 18}
...
```tsx title="src/app/components/ApplicationsTable.tsx" del={11} del="TableCaption,"
import {
  Table,
  TableBody,
  TableCaption,
...
```tsx title="src/app/components/ApplicationsTable.tsx" del={3-6} ins={7-13} startLineNumber=14
<TableHeader>
  <TableRow>
    <TableHead className="w-[100px]">Invoice</TableHead>
    <TableHead>Status</TableHead>
...
```tsx title="src/app/components/ApplicationsTable.tsx" del={3-6} ins={7-15} startLineNumber=25
<TableBody>
  <TableRow>
    <TableCell className="font-medium">INV001</TableCell>
    <TableCell>Paid</TableCell>
...
```tsx title="src/app/components/ui/badge.tsx" startLineNumber=12 del="text-primary-foreground" ins="text-black"
default:
...
```css title="src/app/styles.css" startLineNumber=127
  --color-tag-applied: #b1c7c0;
  --color-tag-interview: #da9b7c;
  --color-tag-new: #db9a9f;
  --color-tag-rejected: #e4e3d4;
...
```tsx title="src/app/components/ui/badge.tsx" collapse={3-10} startLineNumber=10 ins={11-15}
variants: {
  variant: {
    default:
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=34 ins={2-4}
<TableCell>
  <Avatar>
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=35 ins="className="flex items-center gap-2""
...
<Aside type="caution" title="Caching">
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=18 ins="applications={applications}"
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=12 ins={1} ins="applications," ins="applications: Application[];"
...
```tsx title="src/app/components/ApplicationsTable.tsx" ins={2,21} startLineNumber=32
<TableBody>
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=34 ins="key={application.id}"
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=35 ins="{application.status.status}"
<TableCell>
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=6 ins={2-9}
const applications = await db.application.findMany({
  include: {
    status: true,
    company: {
...
- We're `export`ing the type so that we can use it in other files. I like to to put this as close to my Prisma query as possible since the type is directly related to the returned data. Then, we can use this type in other components, like the `ApplicationsTable.tsx` file.
...
If this starts to feel a little overwhelming, this is a great use case for AI. It knows your database schema and query. Ask it!
</Aside>
...
```tsx title="src/app/pages/applications/List.tsx" ins={1-12} startLineNumber={4}
...
```tsx title="src/app/components/ApplicationsTable.tsx" ins={2} ins="ApplicationWithRelations[]" startLineNumber=12 del={1}
import { Application } from "@generated/prisma";
...
If you look at the `TableCell` with the `status` again, you're probably _still_ seeing an error 🤯
...
```tsx title="src/app/components/ui/badge.tsx" startLineNumber=51 "badgeVariants"
export { Badge, badgeVariants };
```

Within our `ApplicationsTable.tsx`:

```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=32 ins=".toLowerCase() as VariantProps<" ins="typeof badgeVariants" ins=">["variant"]"
<Badge
  variant={
...
```tsx showLineNumbers=false ins="badgeVariants" ins={2}
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=47 del={2} ins={3-7}
<TableCell>
  April 15, 2025
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=54 del={2} ins={3}
<TableCell>
  Software Engineer
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=55 del={2} ins={3}
<TableCell>
  RedwoodJS
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=56 del={3,6} ins={4,7}
<TableCell className="flex items-center gap-2">
  <Avatar>
    <AvatarFallback>J</AvatarFallback>
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=67 del={2} ins={3}
<TableCell>
  $150,000-$250,000
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=70 del={2} ins={3}
<TableCell>
  <a href="#">
...
```tsx title="src/app/worker.tsx" startLineNumber=65 ins={6}
layout(
  InteriorLayout,
  prefix("/applications", [
    isAuthenticated,
...
```tsx title="src/app/shared/links.ts" ins={9} collapse={2-8} startLineNumber=3
export const link = defineLinks([
  "/",
  "/user/login",
  "/user/signup",
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=40 del={1}
<pre>{JSON.stringify(applications, null, 2)}</pre>
```
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=39 ins={2-15}
<ApplicationsTable applications={applications} />
...
```tsx title="src/app/pages/applications/List.tsx" showLineNumbers=false ins={1}
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=43 ins="${link("/applications")}?status=archived"
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=49 ins="link("/applications/new")"
<Button asChild>
...
<Aside type="caution" title="Order of Routes">
  The order of the routes within your `worker.tsx` file is important. The code will work it's way through each array, in order, looking for a match.
  ...
  However, if you list the `/applications/new` route before the `/applications/:id` route, it will match the `/applications/new` route first and display the new application page instead of the application details page.
</Aside>
...
```tsx title="src/app/pages/applications/List.tsx" ins={7-8} {6-11,22-27} startLineNumber=31
return (
  <>
    <div className="px-page-side flex justify-between items-center">
      <h1 className="page-title">All Applications</h1>
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=31 del="px-page-side"
return (
    <>
      <div className="px-page-side flex justify-between items-center">
```
...
```tsx title="src/app/pages/applications/List.tsx" del={2,31} ins={3,30} collapse={4-29} startLineNumber=31
return (
  <>
...
<Aside type="tip" title="Vertical Spacing">
  Whenever I'm adjusting the vertical spacing with margins, I always use margin bottom, pushing everything down. A couple of reasons: (1) it adds consistency to my code, making it easier to troubleshoot. (2) There's a "feature" within CSS called **collapsing margins**.
...
```tsx title="src/app/pages/applications/List.tsx" ins="mb-5" ins="mb-8" ins="mb-10" ins={14,16} collapse={4-12} startLineNumber=31
return (
    <div className="px-page-side">
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=19 ins="({ request }: { request: Request })"
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=19 ins={2-3,5}
const List = async ({ request }: { request: Request }) => {
...
![](./images/status-archived-in-terminal.png)
...
```tsx title="src/app/pages/applications/List.tsx" ins={10-12} startLineNumber=25
const applications = await db.application.findMany({
  include: {
    status: true,
    company: {
...
```tsx title="src/app/pages/applications/List.tsx" ins={2,4-8} startLineNumber=52
<div className="mb-8">
...
```tsx title="src/app/pages/applications/List.tsx" ins={2-7, 12} startLineNumber=62
<Button asChild variant="secondary">
  {status === "archived" ? (
