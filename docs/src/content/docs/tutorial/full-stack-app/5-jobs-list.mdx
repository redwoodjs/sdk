...
```css title="src/styles.css" startLineNumber=249
@layer components {
  .page {
    @apply min-h-[calc(100vh-96px)] rounded-xl border-2 border-[#D6D5C5];
  }
...
Let's go ahead and put this on our `ApplicationsList.tsx` page, so we can see the updates we're making in th browser:
...
We've already added the [ShadCN Table component](https://ui.shadcn.com/docs/components/table) to our project, so let's go back to our `ApplicationsTable.tsx` file and use it. Just to start, I'm going to copy the example code from the ShadCN documentation and then we can rework it for our needs:
...
The status column should be a badge. We can reach for the `Badge` component from ShadCN.
...
```tsx title="src/app/components/ui/badge.tsx" startLineNumber=7 "font-bold" "rounded-full"
...
```diff title="src/app/components/ApplicationsTable.tsx" startLineNumber=17
...
```tsx title="src/app/components/ApplicationsTable.tsx" startLineNumber=32 "as VariantProps<typeof badgeVariants>['variant']}"
...
Class Variance Authority is a utility library, used within ShadCN, for creating and using CSS classes.
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=28
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=22 "px-page-side"
<div className="px-page-side flex justify-between items-center">
```

Let's _remove_ this from the `div` and add it to a wrapping `div`.

```tsx title="src/app/pages/applications/List.tsx" "px-page-side"
<InteriorLayout>
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=8
...
```tsx title="src/app/pages/applications/List.tsx" startLineNumber=9
...
```tsx title="src/app/pages/applications/List.tsx" {10-12} startLineNumber=12
const applications = await db.application.findMany({
  include: {
    applicationStatus: true,
    company: {
...
```tsx title="src/app/pages/applications/List.tsx" {2,5-7} startLineNumber=40
<div className="mb-8">
...
```tsx title="src/app/pages/applications/List.tsx" {2-6} startLineNumber=51
<Button asChild variant="secondary">
  {status === "archived" ? (
...
<details>
<summary>Final code for `List.tsx`</summary>
