...
First, let's take a look at our final design in Figma, and what we're building.

![](./images/figma-new-application-form.png)

[In the last section](/tutorial/full-stack-app/jobs-list/), we set up the `/applications/new` route, but we haven't created the page.
...
```tsx title="src/worker.tsx" collapse={2-5} del={11} ins={12} showLineNumbers=false
render(Document, [
...
Conveniently, shadcn/ui has a `Breadcrumb` component that we can use. We've already imported this component into ou project, we just need to use it.
...
```tsx title="src/app/pages/applications/New.tsx" ins={1-8,13-27} del={28}
import {
  Breadcrumb,
  BreadcrumbList,
  BreadcrumbItem,
...
```tsx title="src/app/pages/applications/New.tsx" ins={5-7,16} del={4,9-12,15}
  <Breadcrumb>
    <BreadcrumbList>
      <BreadcrumbItem>
        <BreadcrumbLink href="/">Home</BreadcrumbLink>
...
- I've adjusted the first `BreadcrumbLink` to point to the Applications index or list page.
...
Since we're utilizing the `link` function, we'll need to import it at the top of the file:

```tsx title="src/app/pages/applications/New.tsx"
import { link } from "@/app/shared/links";
```

At the top of the `src/app/components/ui/breadcrumb.tsx` file, we need to add a `use client` directive to specify that our shadcn/ui component is a client component, otherwise our page will error out.

```tsx title="src/app/components/ui/breadcrumb.tsx"
"use client";
```

Let's add some styles to our wrapping `div`:

```tsx title="src/app/pages/applications/New.tsx" collapse={3-13} startLineNumber=12 ins="mb-12 -mt-7 pl-[120px]"
return (
...
```tsx title="src/app/pages/applications/New.tsx" ins={1,15-19} collapse={3-13} startLineNumber=13
<>
  <div className="mb-12 -mt-7 pl-[120px]">
    <Breadcrumb>
      <BreadcrumbList>
...
---
- We can only return a single component, so we need to wrap everything with React fragment (`<></>`)
...
- Then, we have a `p` of text that says "Create a new application." I went ahead and added a class of `page-description` so that we can use this on multiple pages.
---
```

 We need to add a class definition for `page-description` to the `@layer components` section of our `styles.css` file.

```css title="src/app/styles.css" ins={6-8} startLineNumber={165} collapse={2-5}
@layer components {
  .page-title {
    @apply text-3xl;
  }
...
```tsx title="src/app/pages/applications/New.tsx" collapse={2-29} ins={1, 34}
...
```tsx title="src/app/components/ApplicationForm.tsx" ins={2,6} ins="className="grid grid-cols-2 gap-[200px] px-page-side mb-[75px]""
return (
  <form>
...
```tsx title="src/app/components/ApplicationForm.tsx" ins={3-52} startLineNumber=5
<form>
  <div className="grid grid-cols-2 gap-[200px] px-page-side mb-[75px]">
    {/* left side */}
    <div>
...
You'll also notice that on each `p` tag, I added a class of `input-description`. Since there are multiple input descriptions, this will make it easier to style since we can target the class name from our `styles.css` file.
...
```css title="src/app/styles.css" startLineNumber={156}
h2 {
...
```css title="src/app/styles.css" startLineNumber={169} ins={2-4}
@layer components {
  .field {
    @apply pt-8;
  }
...
```css title="src/app/styles.css" startLineNumber={164} ins={1-4}
label,
.label {
...
We already have an `input` definition within our `@layer base`, so we can just add to it. But, it specifically targets the `text` type, so let's extend it to include other input types, as well as, `textarea`.

```css title="src/app/styles.css" startLineNumber={160} ins={1-4}
textarea,
input[type="email"],
input[type="date"],
input[type="url"],
...
Let's add a few more classes for our `textarea`. These need to be listed below our `input` definition, so that we can override some of our `input` styles:

```css title="src/app/styles.css" startLineNumber={168} ins={1-3}
textarea {
  @apply h-[80px] py-3;
}
---
...
```tsx title="src/app/components/ApplicationForm.tsx" startLineNumber=30
<div className="field">
  <div className="label">Salary Range</div>
  <p className="input-description">What does the pay look like?</p>
  <div>
...
```tsx title="src/app/components/ApplicationForm.tsx" startLineNumber=33 ins="flex gap-4"
...
```tsx title="src/app/components/ApplicationForm.tsx" startLineNumber=33 ins="flex-1 label-inside"
<div className="flex gap-4">
...
```css title="src/app/styles.css" collapse={2-5} ins={6-8} startLineNumber={182}
@layer components {
  .field {
    @apply pt-8;
  }
...
```css title="src/app/styles.css" startLineNumber={187} ins={4-10}
.label-inside {
...
```css title="src/app/styles.css" ins={12-14} collapse={2-11} startLineNumber={187}
.label-inside {
  @apply relative flex border border-border outline-1 outline-white rounded-md mb-2 h-9;
...
```css title="src/app/styles.css" startLineNumber={203} ins={1-3}
.input-description {
...
```tsx title="src/app/components/ApplicationForm.tsx" startLineNumber=55 ins={3-6}
{/* right side */}
<div>
  <div className="box">
...
```tsx title="src/app/components/ApplicationForm.tsx" showLineNumbers={false} ins={1}
...
```tsx title="src/app/components/ui/datepicker.tsx" startLineNumber=16 ins="({ name }: { name: string })"
...
```tsx title="src/app/components/ui/datepicker.tsx" collapse={1-22} ins={23} startLineNumber=20
<Popover>
...
```tsx title="src/app/components/ui/datepicker.tsx" ins={2,26} collapse={3-25} startLineNumber=19
  return (
    <>
      <Popover>
        <PopoverTrigger asChild>
...
```tsx title="src/app/components/ApplicationForm.tsx" collapse={2-6} ins={7-17} startLineNumber=57
<div>
  <div className="box">
    <label>Application submission date</label>
    <DatePicker name="dateApplied" />
...
```tsx title="src/app/components/ApplicationForm.tsx" ins={1-7} showLineNumbers=false
import {
  Select,
  SelectContent,
  SelectItem,
...
Let's give a deeper look to the shadcn/ui `Select` component. You can also [check out the official documentation](https://ui.shadcn.com/docs/components/select).

The `Select` component has a trigger (`SelectTrigger`). This is essentially a button that opens the dropdown. Here, I'm using the `placeholder` prop to set the text to "Select a Status."

The content (`SelectContent`) is the dropdown menu. For now, I only have one option in the dropdown, "new". This will eventually need to be dynamic, pulling all our application statuses from our database.

Our `ApplicationForm` component is a client component. But, we need to use a server component in order to fetch all of the application statuses from the database. If we go up the chain, he `ApplicationForm` component's parent is the `New` page component, which is a server component. Let's fetch the data there and pass it into the `ApplicationForm`.

On our `New` page component, we want to import our database and use a Prisma query to get all the applications statuses:

```tsx title="/src/app/pages/applications/New.tsx" startLineNumber=11 ins={1,4} ins="async"
import { db } from "@/db"
...
Still inside the `New.tsx` file, scroll down to the `ApplicationsForm` component and pass in the `statuses` as a prop.

```tsx title="/src/app/pages/applications/New.tsx" startLineNumber={37} ins="statuses={statuses}"
<ApplicationsForm statuses={statuses} />
```

At first, you'll see an error because we need to set up our `ApplicationForm` to receive the data:

```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber=12 ins={1,3} ins="({ statuses }: { statuses: ApplicationStatus[] })"
import { ApplicationStatus } from "@/db"

const ApplicationsForm = ({ statuses }: { statuses: ApplicationStatus[] }) => {
---
You'll notice for the status type, we can reference the `ApplicationStatus` type that Prisma generated for us. It's an array of statuses, so we'll append square brackets `[]`
---
```

Now, we can use the statuses we're passing in to make our `Select` component dynamic. We want to loop over all of our statuses and display a `SelectItem` for each:

```tsx title="/src/app/components/ApplicationForm.tsx" ins={6-10} startLineNumber={73}
<Select name="status">
  <SelectTrigger>
    <SelectValue placeholder="Select a Status" />
  </SelectTrigger>
...
- We're displaying the name of the status `{status.status}` within the browser, but the value that's sent to the server is the `id`.
---
```
...
```css title="/src/app/styles.css" startLineNumber="182" ins={2-4}
@layer components {
  .box {
...
This looks pretty good, but I want to add some spacing between the form label and its input. With the form fields on the left, we have an `input-description` to add space. So, when we're targeting our class within CSS, we want to target labels, but _only_ labels that _don't_ have an `input-description`.
...
```css title="/src/app/styles.css" ins={6-8} startLineNumber={172}
label,
.label {
  @apply text-sm font-medium block;
}
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={16} ins="action={handleSubmit}"
...
```tsx title="/src/app/components/ApplicationForm.tsx" ins={2-9} startLineNumber={14}
...
- By default, this function will receive all the `formData` that the user entered.
- Then, we'll call the `createApplication` function (this doesn't exist yet, we still need to write this) passing in the `formData`. We can save the return value in a variable called `result`.
...
- Otherwise, there's an error and we'll log it to the console.
---
```

Now, let's set up the `createApplication` function we referenced. Create a file for our action. Inside, the `src/app/pages/applications` folder, let's create a new file called `functions.ts`.
...
This is personal preference, but I like co-locating my server actions and functions with the pages that use them.

If it's an action that multiple pages and components use, you can put it in a shared folder like `src/app/lib/functions` and naming the file based on the action.
</Aside>
...
Then, create a function called `createApplication`.
...
```tsx title="/src/app/pages/applications/functions.ts" ins={2-34} startLineNumber={4}
try {
...
---
- You can use the `db.application.create` function from the [`applicationSeed.ts` file](/tutorial/full-stack-app/jobs-list/#option-1-create-a-seed-file) as a starting point.
- We're getting all the data from the form and assigning it to the appropriate database field.
  - The only field that might not be obvious is the `status`. We're getting the `status` value from the form, where it's a string. However, the database needs the `id` to be an integer. So, we're using `parseInt` to convert the string to an integer.
- We haven't set up `contacts` yet, for now we'll hard code in some test data. For everything else, you can use `formData.get("FIELD_NAME")` to get the value of any field. Then, assign it to the appropriate property on the `data` object.
---
```
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={10} {3}
user: {
  connect: {
    id: "f8886f0e-fa1a-485a-9239-e066c0672bf9",
  },
...
```tsx title="/src/app/pages/applications/functions.ts" ins={"1. import package":3} ins={"2. Get the ctx from the requestInfo object": 9} ins={"3. If the user does not exist on the ctx, throw an error": 12} ins={"4. Replace the hard coded user id with the one from the ctx": 21} ins={4, 10, 13-15, 22} showLineNumbers=false
"use server"
