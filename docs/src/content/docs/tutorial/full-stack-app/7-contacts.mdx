...
You can copy and paste the boilerplate code from the [Shad docs](https://ui.shadcn.com/docs/components/sheet):

Import statement for the top of our `ApplicationForm` component (but below the `use client` directive):
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={99}
<div className="box">
  <h3>Contacts</h3>
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={10} "action={handleSubmit}"
<form action={handleSubmit}>
```

Now, we need to write the `handleSubmit` function. We can add this code right above our `return` statement:

```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={10} "handleSubmit"
...
Now, inside our `src/app/pages/applications/actions.ts` file, let's create a new function at the very bottom of our file:

```tsx title="/src/app/pages/applications/actions.ts" startLineNumber={56}
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={21}
const [isContactSheetOpen, setIsContactSheetOpen] = useState(false);
```

Then, be sure to import `useState` at the top of our file:

```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={11}
import { useState } from "react"
```

Now, scroll down to the bottom of the `ApplicationForm` component, to our `Sheet` component:

```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={108} {1, 11} "open={isContactSheetOpen}" "onOpenChange={setIsContactSheetOpen}" "callback={() => setIsContactSheetOpen(false)}"
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={8} {"1. Updated the props we're passing in.": 1} {"2. If we're successful, we'll call the callback function.": 8}
.
...
```tsx title="/src/app/pages/applications/New.tsx" {4-8} startLineNumber={7}
const New = async () => {
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={106} {"1. Check to see if any contacts exist.": 1} {"2. Map over the contacts and display them in a list.": 4} {"3. Temporarily display all the contact details": 6}
.
{contacts && (
  <ul>
    ...
    {contacts.map((contact) => (
      ...
      <li key={contact.id}>{contact.firstName} {contact.lastName} {contact.email} {contact.role}</li>
    ))}
  </ul>
)}
```
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={108}
{contacts.map((contact) => (
...
```tsx title="/src/app/pages/applications/actions.ts" collapse={11-14} startLineNumber={56} {3, 5-7, 15-18}
export const createContact = async (formData: FormData) => {
  try {
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={21} {6}
const ApplicationsForm = ({ statuses, contacts }:
  { statuses: ApplicationStatus[], contacts: Contact[] }) => {
  const [isContactSheetOpen, setIsContactSheetOpen] = useState(false);
...
```tsx title="/src/app/pages/applications/actions.ts" collapse={2-8} startLineNumber={14} {9}
...
```tsx title="/src/app/pages/applications/actions.ts" startLineNumber={4}
import { Contact } from "@prisma/client"
```

Then, when we're creating our company, inside the Prisma `create` method, we can connect our `contacts` to the company:

```diff title="/src/app/pages/applications/actions.ts" startLineNumber={29}
company: {
  create: {
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={8} "handleDelete"
...
```tsx title="/src/app/pages/applications/actions.ts" startLineNumber={82}
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={22} "onClick={(e) => handleDelete(e)}" {3}
<button role="button"
  className="hover:bg-black cursor-pointer text-white fill-current rounded-full bg-destructive p-1"
