...
```tsx title="/src/app/components/ApplicationForm.tsx" ins={3-10}
"use client";
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={10} ins="/app"
} from "@/app/components/ui/sheet"
```
...
Then, copy this code for our side bar:
...
```tsx title="/src/app/components/ApplicationForm.tsx" ins={7-18} startLineNumber={106}
<div className="box">
  <h3>Contacts</h3>
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={113} del={1} ins={2-4}
<SheetTrigger>Open</SheetTrigger>
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={3} ins={1}
...
```tsx title="/src/app/components/ApplicationForm.tsx" del={3,6-7} ins={4,8,10} startLineNumber={118} ins="className="pt-[100px] px-12""
...
- For the form, I'm referencing the `ContactForm` component. (We still need to create this.)
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={3}
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={2}
return (
  <form>
    <div className="field">
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={1} ins={1-2}
import { Icon } from "./Icon"
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={6} ins="action={handleSubmit}"
<form action={handleSubmit}>
```

Now, we need to write the `handleSubmit` function. We can add this code right above our `return` statement:

```tsx title="/src/app/components/ContactForm.tsx" ins={2-9} startLineNumber={4}
const ContactForm = () => {
  ...
  return (
---
- This function will take our form data and pass it to the `createContact` function. (We still need to write this function.)
...
```tsx title="/src/app/pages/applications/functions.ts" startLineNumber={55}
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={1} ins={1}
...
```tsx title="/src/app/components/InteriorLayout.tsx" ins={2, 10}
...
```tsx title="/src/app/components/ui/sonner.tsx" startLineNumber={1} ins={1}
"use client"
```
...
```tsx title="/src/app/components/ContactForm.tsx" del={2,5} ins={3,6} startLineNumber={9}
if (result.success) {
  console.log("Contact created successfully");
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={26} ins={1}
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={117} ins={2-3} ins="open={isContactSheetOpen}" ins="onOpenChange={setIsContactSheetOpen}" ins="callback={() => setIsContactSheetOpen(false)}"
<Sheet
...
```tsx title="/src/app/components/ContactForm.tsx" showLineNumbers=false ins={"1. Updated the props we're passing in.": 1} ins={"2. If we're successful, we'll call the callback function.": 8} ins={2,9}
.
...
```tsx title="/src/app/pages/applications/New.tsx" ins={4-8} startLineNumber={13}
const New = async () => {
  ...
  return (
---
The where object limits the contacts to only those that don't have a company ID. We'll come back later and update this to only include contacts that are associated with the current user.
---
```

Now, we can pass this data into our `ApplicationForm` component:

```tsx title="/src/app/pages/applications/New.tsx" startLineNumber={43} ins="contacts={contacts}"
...
```tsx title="/src/app/components/ApplicationForm.tsx" ins="contacts" ins="contacts: Contact[]" startLineNumber={26}
...
```tsx title="/src/app/components/ApplicationForm.tsx" ins="Contact" startLineNumber={14}
...
```tsx title="/src/app/components/ApplicationForm.tsx" showLineNumbers=false ins={"1. Check to see if any contacts exist.": 2} ins={"2. Map over the contacts and display them in a list.": 5} ins={"3. Temporarily display all the contact details": 7} ins={3, 6, 8-10} del={1}
...
We want to pass in each contact as an object:

```tsx title="/src/app/components/ContactCard.tsx" ins={1} ins="({ contact }: { contact: Contact })"
...
```tsx title="/src/app/components/ContactCard.tsx" del={3} ins={4-17} startLineNumber={4}
return (
  <div>
    ContactCard
    <div>
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={2} ins={1-2}
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={6} ins="className="flex items-center gap-4 mb-6"" ins="className="size-10"" ins="className="flex-1"" ins="className="text-sm font-medium"" ins="className="text-sm text-zinc-500""
return (
...
- **On line 15**, the `flex-1` class ensures that the name and role take up as much horizontal space as possible.
- **On line 16**, `text-sm font-medium` will make the name `14px` and have a font weight of medium.
- **On line 19**, the text role will be `14px` with `text-sm` and gray with `text-zinc-500`.
---
```

Let's swap out our placeholder content with our component. Inside the `ApplicationsForm`:

```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={125} del={3} ins={4}
{contacts.map((contact) => (
  <li key={contact.id}>
    {contact.firstName} {contact.lastName}
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={25} ins={1}
...
```prisma title="/prisma/schema.prisma" collapse={2-8, 11-14, 18-20, 22-23} ins={9-10,21}
model Contact {
...
If you read the error message, the migration file was created, but not applied. You can confirm this by looking in the `migrations` folder in the root of your project. You'll see the new migration file:

![](./images/confirming-new-migration-created.png)
...
The seed command will also reset the database because we clear out all the tables at the top of the script:
...
<Aside title="Running your Migration" type="caution">
When you run the migration, it may prompt you:
...
The only downside is you'll need to create a new user account, by visiting http://localhost:5173/user/signup to register again.
...
```tsx title="/src/app/applicationSeed.ts" startLineNumber={7}
...
Now, you can reference `user.id` within your seed file (assuming at least one user exists _and_ the first user is the user you're testing).
</Aside>
...
```ts title="/src/app/seed/applicationSeed.ts" startLineNumber={29} ins="userId: "c4f35853-8909-4139-98bb-c08663e4230c""
contacts: {
  create: {
    firstName: "John",
    lastName: "Doe",
...
```tsx title="/src/app/pages/applications/functions.ts" collapse={11-14} startLineNumber={55} ins={3, 5-7, 15-18}
export const createContact = async (formData: FormData) => {
  try {
...
---
- In order to get the current user ID, we can get the `ctx` from the `requestInfo` object (line 57). (We already imported `requestInfo` for the `createApplication` function.)
- **On lines 59-61**, we can check to see if the `ctx.user` exists. If not, we'll throw an error.
- If it does, we can connect the `contact` to the `user` using the `connect` method (lines 69-72)
---
```
...
```tsx title="/src/app/pages/applications/New.tsx" startLineNumber={12} ins={1,3,9}
...
- From `ctx`, we can access the `user` object and their ID.
...
![](./images/prisma-studio-modify-contacts-user-id.png)

### Associating the Contact with a Company

Now, when we add a job application, we need to associate the contacts we've added with the company. We'll need to adjust our `createApplication` function within the `src/app/pages/applications/functions.ts` file slightly.
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={27} ins={11}
const ApplicationForm = ({
  statuses,
  contacts,
}: {
...
---
- **On line 27**, we're already passing _all_ of our `contacts` into our `ApplicationForm` component.
...
Be sure to import the `Contact` type at the top of the file from `@/db`:
...
```tsx title="/src/app/pages/applications/functions.ts" del={5-11} ins={12} startLineNumber={30}
company: {
  create: {
...
```tsx title="/src/app/components/ContactCard.tsx" ins={3-7} startLineNumber={6}
return (
  <div className="flex items-center gap-4 mb-6">
    <div>
      <button role="button">
...
---
- On lines **8-12**, we added an `Icon` component with an `id` of `close` and a size of `16px`. Then, we wrapped the `icon` with a `button` and a `div`.
---
```

Now, we need to add some styles.

```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={6} ins="relative" ins="className="absolute top-2 -left-[37px]"" ins="className="rounded-full bg-destructive p-1""
return (
  <div className="relative flex items-center gap-4 mb-6">
    <div className="absolute top-2 -left-[37px]">
      <button role="button" className="rounded-full bg-destructive p-1">
        <Icon id="close" size={16} />
      </button>
    </div>
---
...
I think it'd look better if we made the X white. Since the `x` is an `svg`, we need to target the `fill` attribute.

```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={9} ins="fill-white"
<button role="button" className="fill-white rounded-full bg-destructive p-1">
  <Icon id="close" size={16} />
</button>
```
...
One of my favorite ways to change the fill color of an `svg` is set the `fill` property to the current text color.

You could apply this style, globally, to all SVGs. Within my `styles.css` file:
...
It is worth noting, you can also target an `svg`'s `stroke` property in a similar way:
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={9} ins="hover:bg-black cursor-pointer"
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={7} ins="group/card"
<div className="relative group/card flex items-center gap-4 mb-6">
```

On the `div` wrapping the button:

```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={8} ins="hidden group-hover/card:block"
...
---
- We can set the `div` to `hidden` by default. But, when the user hovers over the `group/card` group it applies a display of `block` with `group-hover/card:block`
---
```

Now, if you test this in the browser, you might feel like you're chasing the button. There's a gap between the where the `div` ends and the `button` begins.

![](./images/gap-between-remove-and-contact-card.png)

Technically, you roll off the `group/card` before you can get to the remove `button`.

We can resolve this by adding some additional padding to the `div` wrapping the `button`. This increases the size of the target area:

```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={8} ins="pr-5"
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={9} ins="onClick={handleDelete}"
<button
...
Now that we're introducing interactivity, this component needs to be a client component. At the top of the file, let's add:

```tsx title="/src/app/components/ContactCard.tsx" ins={1}
"use client"
```

Now, let's create the `handleDelete` function that we referenced in the `onClick` handler. Right above our `return` statement:

```tsx title="/src/app/components/ContactCard.tsx" ins={2-9} startLineNumber={7}
const ContactCard = ({ contact }: { contact: Contact }) => {
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={3} ins={1-2}
import { toast } from "sonner"
import { deleteContact } from "../pages/applications/functions"
```

Now, let's head over to our `functions.ts` file and create the `deleteContact` function. At the very bottom of our file:

```tsx title="/src/app/pages/applications/functions.ts" startLineNumber={82} ins={1-13}
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={22} ins="onClick={(e) => handleDelete(e)}" ins={2}
<button
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={10} ins="e: React.MouseEvent<HTMLButtonElement>" ins="e.preventDefault();" ins={2}
