...
```tsx title="/src/app/pages/applications/New.tsx" startLineNumber={19}
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={10}
<BreadcrumbItem>
...
```diff title="/src/app/pages/applications/Details.tsx" startLineNumber={25}
- <BreadcrumbPage>Add an Application</BreadcrumbPage>
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={38}
<header>
  <div>
  ...
  <div>
    <Button><a href={application?.postingUrl as string} target="_blank">View Application <Icon id="external-link" size={16} /></a></Button>
  </div>
</header>
---
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={40}
...
- We also add some padding (`24px` on teh bottom with `pb-6` and `48px` of margin on the bottom with `mb-12`).
---
```

I want the heading and the badge to appear next to each other, so I'll wrap those elements with an additional `div`.

```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={41}
<div>
...
---
- **On line 42**, I'll use `flex items-center gap-5` to center the heading and badge vertically and add `20px` of space between them. Then, I'll add some `8px` of space to the bottom with `mb-1`.
- **On line 43** we can apply the `page-title` class to the `h1`.
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={51}
...
```tsx title="/src/app/pages/applications/Details.tsx" collapse={3-12} {1, 16} startLineNumber={40} "px-page-side"
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={35} "grid grid-cols-2 gap-[200px]" "mb-[75px]"
return (
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={35} "two-column-grid"
return (
  <form action={handleSubmit}>
...
```tsx title="/src/app/pages/applications/Details.tsx" collapse={9-11} startLineNumber={55}
<div className="two-column-grid">
  <div>
    <div>{application?.jobDescription}</div>
    <div>
      <Button variant="secondary"><Icon id="edit" size={16} /> Edit</Button>
      <Button variant="link"><Icon id="trash" size={16} /> Delete</Button>
    </div>
  </div>
  <aside>
    <p>Sidebar</p>
...
---
- **On line 59** we have our edit button. I'm using the shadcn/ui `Button` component and applied the `secondary` variant to give it a beige color. Inside, I'm using our `Icon` component. I set the `id` to `edit` and the size to `16px`.
...
---
- **On line 57** I added `48px` of margin below our content with `mb-12`
- **On line 58** I adjusted the alignment for the buttons by adding a class of `flex items-center gap-5`. This forces this buttons to align vertically and puts `20px` of spacing between them.
- **On line 60** we can make our delete button red by adding a class of `text-destructive`. Then, to change the color of our svg to match the text color, we can add a class of `fill-current`.
---
```
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={63}
<aside>
  <div className="box">
    <h3>Compensation</h3>
    <div>
      ...
      <div>
        <p>Salary</p>
        <p>{application?.salaryMin} - {application?.salaryMax}</p>
      </div>
    </div>
  </div>
</aside>
---
- **On line 64**, I used the `box` class we've already created.
- We already have some existing styles for `h3`s inside of the `box` class.
- **On line 67** I added an `Icon` component with an `id` of `salary` and a size of `32px`.
- **On line 69-70**, we have a "Salary" label and a `p` tag that contains the salary range. I placed this content in a wrapping `div` so that we can eventually align it vertically with the icon.
---
```
...
---
- **On line 65** I added `16px` of margin below the heading with `mb-4`
- **On line 66**, I aligned the icon and salary information with `flex items-center`. I also added `24px` of padding between with `gap-6`.
- **On line 68** I added a class of `text-sm` to make the salary information `14px`.
- **On line 69** I made the text gray with `text-zinc-500`.
- **On line 70** I added a class of `font-bold` to the salary range to make it bold.
---
```
...
```tsx title="/src/app/components/ApplicationForm.tsx" startLineNumber={105}
<div className="box">
  <h3>Contacts</h3>
...
The only problem is when you hover over a contact card, the delete button appears. I don't want to be able to modify the contact information here.
...
If you look at the DOM that's rendered in the browser, `DialogTrigger` renders as a `button`. Then, we have a `Button` nested inside. To prevent this from happening, we can use the `asChild` prop on the `DialogTrigger`.
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={30}
<DialogFooter>
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={23}
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={32} "onClick={(e) => handleDelete(e)}"
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={19}
...
---
- **On line 20**, we're using the event (`e`) object to prevent the default action of the button.
- **On line 21**, we're calling the `deleteApplication` function (we still need to write this function). This is a server function, we can use `async` / `await`. We'll save the result in a variable called `result`.
- **On line 22**, we're checking to see if the result was successful. If it was, we'll display a successful toast message and redirect the user to the applications list page.
- **On line 25**, otherwise, if we ran into an error, we'll `console.error` the result and display a toast error message.
---
```
...
Within our `action.ts` file, we need to create the `deleteApplication` function.

```tsx title="/src/app/actions.ts" startLineNumber={96}
...
- If there are any errors, they'll be caught on **line 104**, `console.error` out. Then, we'll return `success` as `false` and include the `error` in the response.
---
```

Now, let's go back to our `Details.tsx` file and add our `DeleteApplicationButton` component. We can replace the "Delete" `Button` with our `DeleteApplicationButton` component and pass in the `application?.id` as the `applicationId` prop. As always, be sure to import `DeleteApplicationButton` at the top of your file:

```diff title="/src/app/pages/applications/Details.tsx" showLineNumbers=false
...
+ const Edit = async ({ ctx }: { ctx: AppContext }) => {
...
- export { New }
+ export { Edit }
```
...
```tsx title="/src/worker.tsx" startLineNumber={74}
prefix("/applications", [
  route("/", [isAuthenticated, List]),
  route("/new", [isAuthenticated, New]),
  route("/:id", [isAuthenticated, Details]),
...
```tsx title="/src/app/shared/links.ts" {11} collapse={2-10} startLineNumber={10}
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={68}
...
```tsx title="/src/app/pages/applications/Edit.tsx" startLineNumber={8}
...
- **On lines 10-21** we're using the Prisma API to find the unique application, `where` the `id` is equal to `param.id`.
...
```tsx title="/src/app/pages/applications/Edit.tsx" startLineNumber={24}
...
```tsx title="/src/app/pages/applications/Edit.tsx" {6-14} collapse={2-5} startLineNumber={29}
<BreadcrumbList>
  <BreadcrumbItem>
    <BreadcrumbLink href="/applications">Applications</BreadcrumbLink>
  </BreadcrumbItem>
...
```tsx title="/src/app/pages/applications/Edit.tsx" startLineNumber={47} "Edit {application?.jobTitle}" "Edit your application"
...
```tsx title="/src/app/components/EditApplicationForm.tsx" {5-7} startLineNumber={75} "flex items-center gap-5"
...
- **On line 79**, I gave the Cancel button a `variant` of `secondary` to give it a `beige` color. The `asChild` prop allows us to use the `a` tag inside.
- **On line 80**, I used our `link` function to reference the job details page and pass in the `application?.id` as the `id` parameter.
---
```
...
```tsx title="/src/app/components/ui/DatePicker.tsx" showLineNumbers=false "defaultValue ? new Date(defaultValue) : new Date()"
...
```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={93}
<Select name="statusId" defaultValue={application?.applicationStatus?.status ?? ''}>
```
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={8} "companyId = ''" "companyId?: string" startLineNumber={8}
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={10} {2}
...
```tsx title="/src/app/pages/applications/actions.tsx" collapse={2-8} startLineNumber={53} {9}
...
```tsx title="/src/app/pages/applications/actions.ts" collapse={2-11} {12-18} startLineNumber={63}
...
```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={25}
const handleSubmit = async (formData: FormData) => {
...
Inside our `src/app/pages/applications/actions.ts` file, let's write the `updateApplication` function:

```tsx title="/src/app/pages/applications/actions.ts" startLineNumber={90}
...
- **On line 92** - Our database call is similar to the `createApplication` function, but instead of `create`, we're using `update`.
- **On lines 93-95** - We're using a `where` clause to specify which application we want to update.
- **On lines 103-107** - We're using an `update` method to update the company name (instead of `connect`)
- **On lines 108-112** - We're using the `update` method to update the application status (instead of `connect`).
- **On line 110** - the Application Status ID is a number, but the `statusId` on the `formData` is a `number`, so we're casting it from a string into a number with `parseInt`.
---
```
