...
```tsx title="/src/worker.tsx" collapse={4-7} ins={1,15} del={14}
...
It's pretty standard, but we have a wrapping `div` with styles applied for placement. Let's abstract out the `mb-12 -mt-7 pl-[120px]` styles into a CSS class so that we can use it on both pages. If we need to change it, we only have to do it in one place. In our `styles.css` file, at the bottom of the `@layer components` section:

```css title="/src/styles.css" startLineNumber={231} ins={1-3}
.breadcrumbs {
...
```tsx title="/src/app/pages/applications/New.tsx" ins="breadcrumbs" collapse={2-12} startLineNumber={26}
...
Then, let's copy this code and paste it into our `Details.tsx` file:

```tsx title="/src/app/pages/applications/Details.tsx" del={1} ins={2-18} startLineNumber={2}
return <div>Details</div>;
return (
  <>
    <div className="breadcrumbs">
...
```tsx title="/src/app/pages/applications/Details.tsx" ins={1-8}
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={16} del={2} ins={3}
<BreadcrumbItem>
  <BreadcrumbLink href="/applications">Applications</BreadcrumbLink>
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={3} ins={1} ins="{ params }: RequestInfo"
import { RequestInfo } from "rwsdk/worker"

const Details = ({ params }: RequestInfo) => {
```
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={11} ins={2}
const Details = ({ params }: RequestInfo) => {
  console.log({ params });
  return (
```
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={10} ins={1,4-8,11} ins="async" del={10}
import { db } from "@/db"
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={30} ins={1} del={2-5}
<BreadcrumbPage>Add an Application</BreadcrumbPage>
<BreadcrumbPage>
...
```tsx title="/src/app/pages/applications/Details.tsx" ins={5-7} startLineNumber={13}
const application = await db.application.findUnique({
  where: {
    id: params.id,
  },
...
```tsx title="/src/app/pages/applications/Details.tsx"  ins={6-10} startLineNumber={13}
const application = await db.application.findUnique({
  where: {
    id: params.id,
  },
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={27} del={1}
console.log({ application });
```
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={45}
<header>
  <div>
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={51} ins={2,13} ins="className="page-title" ins="<span className="text-zinc-500">" ins="</span>"
<div>
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={67} ins={5}
<Button>
  <a
...
```tsx title="/src/app/pages/applications/Details.tsx" collapse={3-12} ins={1, 16} startLineNumber={49} ins="px-page-side"
...
```css title="/src/styles.css" startLineNumber={235} ins={1-3}
.two-column-grid {
  @apply grid grid-cols-2 gap-[200px] mb-[75px]
}
```

Back in our `Details.tsx` file, we can use our new class. At the bottom of our JSX:

```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={81} ins={1-6}
<div className="px-page-side two-column-grid">
  <p>Details</p>
  <aside>
    <p>Sidebar</p>
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={54} del={2} ins={3-5}
...
```tsx title="/src/app/pages/applications/Details.tsx" ins={2, 4-12} startLineNumber={81}
<div className="two-column-grid">
  <div>
...
```tsx title="/src/app/pages/applications/Details.tsx" collapse={13-15} startLineNumber={81} ins="mb-12" ins="flex items-center gap-5" ins="className="text-destructive"" ins="className="fill-destructive""
<div className="two-column-grid">
  <div>
    ...
    <div className="flex items-center gap-5">
      ...
      <Button variant="link" className="text-destructive">
        <Icon id="trash" size={16} className="fill-destructive" /> Delete
      </Button>
    </div>
  </div>
  <aside>
...
```tsx title="/src/app/pages/applications/Details.tsx" ins={2-13} startLineNumber={93}
<aside>
  <div className="box">
    <h3>Compensation</h3>
    <div>
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={93} ins="mb-4" ins="flex items-center gap-6" ins="text-sm" ins="text-zinc-500" ins="font-bold"
<aside>
  <div className="box">
...
```tsx title="/src/app/pages/applications/Details.tsx" collapse={2-13} ins={14-24} startLineNumber={93}
<aside>
  <div className="box">
    <h3 className="mb-4">Compensation</h3>
    <div className="flex items-center gap-6">
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={113} ins="application?.company?.contacts"
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={9} ins={3,6}
const ContactCard = ({
  contact,
  isEditable = true,
}: {
...
```tsx title="/src/app/components/ContactCard.tsx" startLineNumber={28} ins={1, 11}
{isEditable && (
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" ins={1,3-11,15-26}
"use client";
...
export { DeleteApplicationButton }
---
- You'll notice I included the `use client` directive at the top of our file, as well as our imports. If you copied the `import` statement directly [from shadcn/ui](https://ui.shadcn.com/docs/components/dialog), the file paths will vary and need to be updated.
---
```

Before, I forget, we also need to go into our `app/components/ui/dialog.tsx` shadcn/ui component and add a `use client` directive to the top of our shadcn/ui component as well:

```tsx title="/src/app/components/ui/dialog.tsx" ins={1}
"use client";
```

Back inside our `DeleteApplicationButton` component, let's start customizing it, to meet our needs.

Let's delete the `Open` text and insert the `Delete` button. You can use the `Delete` button from our `Details.tsx` file (line 89). It's already styled:

```tsx title="/src/app/components/DeleteApplicationButton.tsx" showLineNumbers=false del={2} ins={3-5}
<DialogTrigger>
  Open
...
You'll also need to import the `Button` and `Icon` components at the top of your file:

```tsx title="/src/app/components/DeleteApplicationButton.tsx" showLineNumbers=false ins={1-2}
...
```tsx title="/src/app/pages/applications/Details.tsx" showLineNumbers=false ins={1}
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={18} ins="asChild"
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={25} ins="Are you absolutely sure?"
<DialogTitle>Are you absolutely sure?</DialogTitle>
```
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={25} ins={2}
<DialogDescription>
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" collapse={2-8} ins={9-15} startLineNumber={31}
<DialogContent>
  <DialogHeader>
    <DialogTitle>Are you absolutely sure?</DialogTitle>
    <DialogDescription>
...
---
- For the "Nevermind" button, use the `secondary` variant, applying a beige background color.
- For the "Yes, Delete It" button, set the `variant` to `destructive`. Inside, add an `Icon` component with an `id` of `check`.
---
```

Let's go back through and add a little more styling:

```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={23} ins="py-12 px-14" ins="mb-4" ins="text-destructive text-3xl font-bold mb-2" ins="fill-current"
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={35} ins={4}
<Button
  variant="destructive"
...
- **On line 18**, we're calling the `deleteApplication` function (we still need to write this function). This will be a server function, we can use `async` / `await`. We'll save the result in a variable called `result`.
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" ins="applicationId" startLineNumber={17}
const DeleteApplicationButton = ({
  applicationId,
}: {
...
```tsx title="/src/app/functions.ts" startLineNumber={96}
...
---
- **On line 96**, we're `export`ing the `deleteApplication` function so that we can use it in our `DeleteApplicationButton` component. This is an `async` function, so we can use `await` to wait for the database call to complete.
- We using a `try` / `catch` block to run the Prisma API call.
- **On line 98**, we're running the calling the `delete` method, `where` the `id` matches the `applicationId` that we passed in.
- If we're successful, we'll return `success` as `true` and `error` as `null`.
- If there are any errors, they'll be caught. On **line 105**, `console.error` out. Then, we'll return `success` as `false` and include the `error` in the response.
---
```
...
```tsx title="/src/app/pages/applications/Details.tsx" startLineNumber={90} ins="applicationId={application?.id as string}"
...
```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={33} ins="open={isOpen} onOpenChange={setIsOpen}"
<Dialog open={isOpen} onOpenChange={setIsOpen}>
```

Now, we can update the "Nevermind" button to close the dialog box, by updating the `isOpen` state:

```tsx title="/src/app/components/DeleteApplicationButton.tsx" startLineNumber={55} ins="onClick={() => setIsOpen(false)}>"
...
```tsx title="/src/app/pages/applications/Edit.tsx" showLineNumbers=false del={1,4} ins={2,5}
...
```tsx title="/src/worker.tsx" ins={8} collapse={5-7} startLineNumber={67}
layout(
  InteriorLayout,
  prefix("/applications", [
    isAuthenticated,
...
```tsx title="/src/app/shared/links.ts" ins={11} collapse={2-10} startLineNumber={3}
...
```tsx title="/src/app/pages/applications/Details.tsx" ins={2-6,8} startLineNumber={87} ins="asChild"
...
---
- Update the `Button` component to include the `asChild` prop. This maintains the button styles, but renders a link instead.
- Inside the `Button` component, add a link, that uses the `link` function to link to the `Edit` page. The first parameter is the route and the second parameter contains an object with the appropriate parameters. Here, the `application?.id`
---
```
...
![](./images/edit-application-looks-like-new.png)
...
```tsx title="/src/app/pages/applications/Edit.tsx" ins="params," ins={2-14} startLineNumber={14}
...
- We can access the `id` through the URL by referencing the `params` object with `params.id`.
...
```tsx title="/src/app/pages/applications/Edit.tsx" collapse={2-5} del={13} ins={5-10,14} startLineNumber={34}
<BreadcrumbList>
  <BreadcrumbItem>
    <BreadcrumbLink href="/applications">Applications</BreadcrumbLink>
  </BreadcrumbItem>
...
---
- Inject a `BreadcrumbItem` and a `BreadcrumbSeparator` after the Applications link to include the job details page.
- On the `BreadcrumbLink`, use the `link` function to reference the job details page.
- Use the content from our database to dynamically display the job title and company name.
- Then, change the `BreadcrumbPage` to `Edit Application`.
---
```
...
```tsx title="/src/app/pages/applications/Edit.tsx" startLineNumber={52}
...
```tsx title="/src/app/pages/applications/Edit.tsx" startLineNumber={52} ins="Edit {application?.jobTitle}" ins="Edit your application" ins="Edit the details of this job application."
<div className="mx-page-side pb-6 mb-8 border-b-1 border-border">
...
```tsx title="/src/app/components/EditApplicationForm.tsx" showLineNumbers=false ins={2,5} del={1,4}
...
```tsx title="/src/app/pages/applications/Edit.tsx" showLineNumbers=false ins={2,5} del={1,4}
...
```tsx title="/src/app/pages/applications/Edit.tsx" showLineNumbers=false del="contacts={contacts}" ins="application={application}" ins={2} del={1}
<EditApplicationForm statuses={statuses} contacts={contacts} />
<EditApplicationForm statuses={statuses} application={application as ApplicationWithRelations} />
```

Then, be sure to import the `ApplicationWithRelations` type at the top of the file:

```tsx title="/src/app/pages/applications/Edit.tsx" showLineNumbers=false ins={1}
import { ApplicationWithRelations } from "./List";
```

Now, let's move over to our `EditApplicationForm.tsx` file and update the props, there, to match:

```tsx title="/src/app/components/EditApplicationForm.tsx" showLineNumbers=false del={1,5-6} ins={2-3,7-13}
import { ApplicationStatus, Contact } from "@/db";
import { ApplicationStatus, Application } from "@/db"
...
Update the types that we're importing from Prisma to remove `Contact` and include `Application`. Then, we made similar changes to the props that we're passing into our component.

Notice that we imported `ApplicationWithRelations` from our `List` page. This is a type that we created to include the `company` and `contacts` details.
...
```tsx title="/src/app/components/EditApplicationForm.tsx" showLineNumbers=false ins={"Company Name": 1} ins="defaultValue={application?.company?.name ?? ''}" ins={"Job Title": 3} ins="defaultValue={application?.jobTitle ?? ''}" ins={"Job Description": 5} ins="defaultValue={application?.jobDescription ?? ''}" ins={"Salary Min": 7} ins="defaultValue={application?.salaryMin ?? ''}" ins={"Salary Max": 9} ins="defaultValue={application?.salaryMax ?? ''}" ins={"Posting URL": 11} ins="defaultValue={application?.postingUrl ?? ''}"
.
...
```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={120} ins="Update"
<Button role="submit">
  Update
</Button>
```

Let's add a second button, for cancelling changes:

```tsx title="/src/app/components/EditApplicationForm.tsx" ins={5-7} startLineNumber={119} ins="field flex items-center gap-4" ins="asChild"
...
---
- On the wrapping `field` `div`, add a few classes to align the buttons vertically and add `16px` of space between them with `flex items-center gap-4`.
- **On line 123**, give the Cancel button a `variant` of `secondary` to give it a `beige` color. The `asChild` prop allows us to use the `a` tag inside.
- **On line 124**, use our `link` function to reference the job details page and pass in the `application?.id` as the `id` parameter.
---
```
...
```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={139} ins="defaultValue={application?.dateApplied?.toISOString() ?? ""}"
<DatePicker
  name="dateApplied"
...
```tsx title="/src/app/components/ui/DatePicker.tsx" startLineNumber={16} ins={3,6}
...
```tsx lang="tsx" title="/src/app/components/ui/DatePicker.tsx" showLineNumbers=false ins={2}
...
```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={147} ins={3}
<Select
  name="status"
...
```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={171} del={1,4} ins={2,5}
{contacts && (
{application?.company?.contacts && (
  <ul>
    {contacts.map((contact) => (
...
```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={195} ins={3}
<ContactForm
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={6} ins={3,6}
const ContactForm = ({
  callback,
  companyId = "",
}: {
...
---
- Set the default value to an empty string: `companyId = ''`
- Make the `companyId` prop optional with a question mark `?`
...
```tsx title="/src/app/components/ContactForm.tsx" startLineNumber={13} ins={2}
...
```tsx title="/src/app/pages/applications/functions.tsx" collapse={2-8} startLineNumber={54} ins={9}
...
```tsx title="/src/app/pages/applications/actions.ts" collapse={2-11} ins={12-18} startLineNumber={64}
...
---
- **On line 75**, we're using a spread operator to say if the `companyId` exists, then connect the company to the contact. Otherwise, we'll use an empty object.
---
```
...
We can remove line `39`, where we're appending the `contacts` to the `formData`.

```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={39} del={1}
formData.append("contacts", JSON.stringify(contacts));
```

Instead of calling `createApplication`, let's call `updateApplication` instead (we still need to write this function):

```tsx title="/src/app/pages/applications/actions.ts" collapse={4-8} startLineNumber={126} del={2} ins={3}
const handleSubmit = async (formData: FormData) => {
  const result = await createApplication(formData);
...
```tsx title="/src/app/components/EditApplicationForm.tsx" showLineNumbers=false del={1} ins={2}
...
```tsx title="/src/app/pages/applications/functions.ts" startLineNumber={121}
...
- **On line 123** - Our database call is similar to the `createApplication` function, but instead of `create`, we're using `update`.
- **On lines 124-126** - We're using a `where` clause to specify which application we want to update.
- **On lines 134-138** - We're using an `update` method to update the company name (instead of `connect`)
- **On lines 139-143** - We're using the `update` method to update the application status (instead of `connect`).
- **On line 141** - the Application Status ID is a number, but the `statusId` on the `formData` is a `number`, so we're casting it from a string into a number with `parseInt`.
---
```
...
```tsx title="/src/app/components/EditApplicationForm.tsx" startLineNumber={119} ins={2-7}
<div className="field flex items-center gap-4">
<input
  type="hidden"
  id="id"
