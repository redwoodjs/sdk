...
You may need to update the security headers to allow your Cloudflare URL.

Within the content loading and script execution section, add the `workers.dev` or your custom domain name:

```ts title="src/app/headers.ts" "https://applywize.redwoodjs.workers.dev" startLineNumber={27}
