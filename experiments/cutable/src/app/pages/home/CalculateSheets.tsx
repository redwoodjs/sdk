...
export function CalculateSheets() {
  ...
  useEffect(() => {
    ...
    if (
      JSON.stringify(calculatedSheets) === JSON.stringify(prevBoardsRef.current)
    ) {
      return;
    }
  ...
  const calculateCuts = async () => {
    ...
    try {
      ...
      const boards = await calculateCutsAction(
        flatPanels,
        sheetWidth,
        sheetLength,
        bladeWidth,
  ...
  const averageEfficiency = calculatedSheets.length
    ? calculatedSheets.reduce((acc, sheet) => acc + sheet.efficiency, 0) /
      calculatedSheets.length
    : 0;
  ...
  const generateRandomPanels = async () => {
    const randomPanels: Panel[] = Array.from({ length: 6 }, () => ({
      width:
        Math.round(Math.random() * (1000 - 200) + 200) -
        (Math.round(Math.random() * (1000 - 200) + 200) % 50),
      length:
        Math.round(Math.random() * (1200 - 200) + 200) -
        (Math.round(Math.random() * (1200 - 200) + 200) % 50),
      ...
      <div className="w-[400px] border-r p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-[200px] bg-white rounded-md shadow-lg border border-gray-200 p-1 z-50">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {panels.map((panel, index) => (
              <div
                key={index}
                className="grid grid-cols-[2fr_2fr_1fr_auto_auto] gap-2 items-center"
              >
                <Input
                  type="number"
                  value={panel.width}
                  onChange={(e) =>
                    updatePanel(index, "width", Number(e.target.value))
                  }
                  className="h-8"
                />
                <Input
                  ...
                  value={panel.length}
                  onChange={(e) =>
                    updatePanel(index, "length", Number(e.target.value))
                  }
                  className="h-8"
                />
                <Input
                  ...
                  onChange={(e) =>
                    updatePanel(index, "quantity", Number(e.target.value))
                  }
                  className="h-8"
                />
                <Button
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                <div>
                  ...
                  <span className="font-medium ml-2">
                    {sheetWidth}mm × {sheetLength}mm
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground">Efficiency:</span>
                  <span className="font-medium ml-2">
                    {(board.efficiency * 100).toFixed(1)}%
                  </span>
                </div>
              </div>
            </div>
