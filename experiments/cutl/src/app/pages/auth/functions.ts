...
export async function emailLoginLink(email: string) {
  ...
  const loginUrl = `${getEnv().APP_URL}${link('/auth/callback')}?token=${token}&email=${encodeURIComponent(email)}`;
  console.log('### loginUrl', loginUrl)
  const resend = new Resend(getEnv().RESEND_API_KEY);
  console.log('### resend')
  await resend.emails.send({
    from: "Cutle <auth@cutle.me>",
    to: email,
    subject: `Login to Cutle ${new Date().toLocaleTimeString()}`,
    html: `
    <h1>Login to Cutle</h1>
    <p>Click the link to log in to your account:</p>
    <a href="${loginUrl}">Login to Cutle</a>
