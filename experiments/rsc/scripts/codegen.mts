...
export const codegen = async () => {
  ...
  const latestSqliteFile = (await $`find ./.wrangler/state/v3/d1/ -name *.sqlite -exec ls -t {} +`).stdout.split('\n')[0]
  ...
  await $({ stdio: 'inherit' })`pnpm kysely-codegen --dialect sqlite --url ${latestSqliteFile}`
}
