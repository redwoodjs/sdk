...
const configs = {
  ...
  workerDevServer: ({ context }: { context: DevServerContext }): InlineConfig =>
    mergeConfig(viteConfigs.workerBase(), {
...
const setup = async (): Promise<DevServerContext> => {
  ...
  const viteWorkerDevServer = await createViteServer(
    configs.workerDevServer({ context: context as DevServerContext }),
  );
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (
        ...
        const webResponse = await context.miniflare.dispatchFetch(
          webRequest.url,
