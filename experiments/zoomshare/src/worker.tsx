...
const app = defineApp<Context>([
  ...
  route("/webhook/meeting.recording.completed", [
    ...
    async function ({ request, env }) {
      ...
      const message = { version: '2025-02-26', action: 'download', recordings: testPayload.payload.object.recording_files, downloadToken: testPayload.download_token }
    ...
    for (const message of batch.messages) {
      if (message.body.action === 'download') {
        for (const recording of message.body.recordings) {
          ...
          const r = await env.R2.put(filename, f.body)
        }
      }
    }
  }
