...
export function registerSyncedStateHandlers(
  ...
  SyncedStateServer.registerSetStateHandler((key: string, value: unknown, stub: DurableObjectStub<SyncedStateServer>) => {
    ...
    console.log('[setStateHandler] updated globalState, allKeys:', Object.keys(globalState));
  ...
  SyncedStateServer.registerGetStateHandler((key: string, value: unknown, stub: DurableObjectStub<SyncedStateServer>) => {
    ...
    globalState[key] = value;
    console.log('[getStateHandler] updated globalState, allKeys:', Object.keys(globalState));
    // Sync to the Durable Object for client access
    void syncGlobalState(stub, globalState);
  });

  SyncedStateServer.registerKeyHandler(async (key: string, stub: DurableObjectStub<SyncedStateServer>) => {
  ...
  SyncedStateServer.registerSubscribeHandler((key: string, stub: DurableObjectStub<SyncedStateServer>) => {
  ...
  SyncedStateServer.registerUnsubscribeHandler((key: string, stub: DurableObjectStub<SyncedStateServer>) => {
    const userId = requestInfo.ctx.userId
    if (key === "presence" && userId) {
      console.log('updatePresenceList', userId)
