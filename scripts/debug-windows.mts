...
async function main() {
  ...
  console.log(`Triggering the 'Windows Debug Session' workflow on branch '${branch}'...`);
  ...
  for (let i = 0; i < 5; i++) {
    try {
      const result = execSync(`gh run list --workflow="windows-debug.yml" --branch="${branch}" --limit 1 --json databaseId`).toString();
  ...
  if (!runId) {
    console.error("Could not find a recent workflow run. Please check the Actions tab in your repository.");
    process.exit(1);
  }

  const runUrl = execSync(`gh run view ${runId} --json url -q .url`).toString().trim();
  ...
  while (Date.now() - startTime < timeout) {
    try {
      ...
      const statusResult = execSync(`gh run view ${runId} --json status -q .status`).toString().trim();
      if (statusResult === 'completed') {
        console.error("The workflow run completed without providing an SSH connection.");
  ...
  if (!sshCommand) {
    console.error("The workflow timed out without providing an SSH connection string.");
