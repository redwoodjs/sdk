...
for i in {1..30}; do # Poll for up to 5 minutes (30 * 10 seconds)
  # Turn off exit on error temporarily to handle the case where logs aren't ready
  set +e
  ...
  if [ $GH_EXIT_CODE -ne 0 ]; then
    echo "Logs not yet available. Retrying... (attempt $i of 30)"
    sleep 10
    continue
  fi
  ...
  if [ -n "$SSH_CONNECTION_LINE" ]; then
    # Strip the prefix to get the raw command
    RAW_SSH_COMMAND=$(echo "$SSH_CONNECTION_LINE" | sed 's/.*::notice title=tmate SSH session:://')
    # Use pure bash parameter expansion to remove the carriage return. This is more portable than sed/tr.
    SSH_COMMAND="${RAW_SSH_COMMAND//$'\r'/}"
