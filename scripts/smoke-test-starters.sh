...
cd "$SDK_ROOT/sdk"
if ! pnpm smoke-test "/" -p="$SDK_ROOT/starters/minimal" --artifact-dir="$ARTIFACT_DIR/minimal"; then
...
cd "$SDK_ROOT/sdk"
if ! pnpm smoke-test "/user/login" -p="$SDK_ROOT/starters/standard" --artifact-dir="$ARTIFACT_DIR/standard"; then
