...
cd "$SDK_ROOT/sdk"
if ! pnpm smoke-test --url="/" --path="$SDK_ROOT/starters/minimal" --artifact-dir="$ARTIFACT_DIR/minimal"; then
...
cd "$SDK_ROOT/sdk"
if ! pnpm smoke-test --url="/user/login" --path="$SDK_ROOT/starters/standard" --artifact-dir="$ARTIFACT_DIR/standard"; then
