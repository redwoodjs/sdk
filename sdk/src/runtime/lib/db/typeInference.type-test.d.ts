...
import type { Expect, Equal } from "./type-tests/test-utils";
...
(test = "renameTable") => {
  const migrations = {
    ...
    "1": {
      async up(db: Kysely<any>) {
        await db.schema.renameTable("users", "users_new").execute();
      },
    },
  } as const;
...
(test = "dropColumn") => {
  const migrations = {
    ...
    "1": {
      async up(db: Kysely<any>) {
        await db.schema.alterTable("users").dropColumn("posts").execute();
      },
    },
  } as const;
