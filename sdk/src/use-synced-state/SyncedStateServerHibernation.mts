...
export class SyncedStateServerHibernation extends DurableObject {
  ...
  async webSocketMessage(ws: WebSocket, data: string | ArrayBuffer) {
    ...
    switch (message.kind) {
      case "getState": {
        const value = this.#getState(storageKey);
        this.#send(ws, {
          kind: "getState",
          key: message.key,
          value,
      ...
      case "setState": {
        this.#setState(storageKey, message.value);
        this.#send(ws, {
          kind: "setState",
          key: message.key,
          id: message.id,
  ...
  #getState(key: string): SyncedStateValue | undefined {
  ...
  #setState(key: string, value: SyncedStateValue): void {
